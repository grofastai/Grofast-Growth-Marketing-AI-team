    ...
    <div className="p-4 md:p-6 xl:p-8 max-w-[1400px]">
      ...
      <div className="flex flex-wrap items-start justify-between gap-3 mb-7">
        <div>
          ...
          <p className="text-sm mt-1" style={{ color: "#1b365d" }}>Media team shoot schedules and logs</p>
        </div>
      ...
      <div className="grid grid-cols-2 lg:grid-cols-4 gap-3 mb-6">
        {[
          ...
          <div key={s.label} className="rounded-xl p-4" style={{ background: '#FFFFFF', border: '1px solid #E5E7EB' }}>
            ...
            <p className="text-[11px] font-medium" style={{ color: "#1b365d" }}>{s.label}</p>
          </div>
        ))}
      </div>
      ...
      {shoots.length === 0 ? (
        <div className="flex flex-col items-center py-20 rounded-2xl"
          ...
          <p className="text-[13px] font-semibold" style={{ color: "#1b365d" }}>No shoots logged yet</p>
          <p className="text-[12px] mt-1" style={{ color: "#1b365d" }}>Add the first shoot above.</p>
        </div>
      ) : (
                  ...
                  <div className="flex-1 min-w-0">
                    <div className="flex items-start justify-between gap-3 flex-wrap">
                      <div>
                        ...
                        <p className="text-[13px] font-semibold mt-0.5" style={{ color: "#1b365d" }}>
                          {shoot.client}
                        </p>
                      </div>
                      ...
                      <div className="flex items-center gap-2 flex-shrink-0">
                        ...
                        <button
                          ...
                          {pending && actionId === shoot.id + 'del'
                            ? <Loader2 size={13} className="animate-spin" style={{ color: "#1b365d" }} />
                            : <Trash2 size={13} style={{ color: "#1b365d" }} />}
                        </button>
                      </div>
                    </div>
                    ...
                    <div className="mt-3 grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-2">
                      <div className="flex items-start gap-1.5">
                        <MapPin size={12} className="mt-0.5 flex-shrink-0" style={{ color: "#1b365d" }} />
                        <span className="text-[12px]" style={{ color: "#1b365d" }}>{shoot.location}</span>
                      </div>
                      <div className="flex items-start gap-1.5">
                        <Clock size={12} className="mt-0.5 flex-shrink-0" style={{ color: "#1b365d" }} />
                      ...
                      {shoot.team_assigned && (
                        <div className="flex items-start gap-1.5">
                          <Users size={12} className="mt-0.5 flex-shrink-0" style={{ color: "#1b365d" }} />
                          <span className="text-[12px]" style={{ color: "#1b365d" }}>{shoot.team_assigned}</span>
                        </div>
                      )}
                      {shoot.equipment_used && (
                        <div className="flex items-start gap-1.5">
                          <Wrench size={12} className="mt-0.5 flex-shrink-0" style={{ color: "#1b365d" }} />
                          <span className="text-[12px]" style={{ color: "#1b365d" }}>{shoot.equipment_used}</span>
                        </div>
                      )}
                    </div>
                    ...
                    <div className="flex items-center gap-4 mt-3 pt-3 flex-wrap"
                      ...
                      <div className="flex items-center gap-1.5">
                        <IndianRupee size={11} style={{ color: "#1b365d" }} />
                        <span className="text-[12px] font-semibold" style={{ color: "#1b365d" }}>
                          ₹{(shoot.travel_expense ?? 0).toLocaleString('en-IN')} travel
                        </span>
                      </div>
                      {(shoot.travel_time_hours ?? 0) > 0 && (
                        <div className="flex items-center gap-1.5">
                          <Clock size={11} style={{ color: "#1b365d" }} />
                          <span className="text-[12px] font-semibold" style={{ color: "#1b365d" }}>
                      ...
                      {shoot.creator && (
                        <span className="text-[11px]" style={{ color: "#1b365d" }}>
          ...
          <div className="fixed inset-0 z-50 flex items-center justify-center p-4">
            <div className="w-full max-w-[500px] rounded-2xl shadow-2xl max-h-[90vh] overflow-y-auto"
              ...
              <div className="px-6 py-5 space-y-4">
                {/* Title */}
                <div>
                  <label className="block text-[10px] font-bold uppercase tracking-[0.15em] mb-1.5" style={{ color: "#1b365d" }}>Shoot Title *</label>
                  <input placeholder="e.g. Product Launch Campaign" style={FIELD} value={form.title} onChange={set('title')} />
                </div>

                {/* Client */}
                <div>
                  <label className="block text-[10px] font-bold uppercase tracking-[0.15em] mb-1.5" style={{ color: "#1b365d" }}>Client *</label>
                  <input placeholder="Client name" style={FIELD} value={form.client} onChange={set('client')} />
                </div>

                {/* Location */}
                <div>
                  <label className="block text-[10px] font-bold uppercase tracking-[0.15em] mb-1.5" style={{ color: "#1b365d" }}>Location *</label>
                ...
                <div className="grid grid-cols-2 gap-3">
                  <div>
                    <label className="block text-[10px] font-bold uppercase tracking-[0.15em] mb-1.5" style={{ color: "#1b365d" }}>Start Time *</label>
                    <input type="datetime-local" style={{ ...FIELD, colorScheme: 'light' }} value={form.start_time} onChange={set('start_time')} />
                  </div>
                  <div>
                    <label className="block text-[10px] font-bold uppercase tracking-[0.15em] mb-1.5" style={{ color: "#1b365d" }}>End Time *</label>
                ...
                {/* Team */}
                <div>
                  <label className="block text-[10px] font-bold uppercase tracking-[0.15em] mb-1.5" style={{ color: "#1b365d" }}>Team Assigned</label>
                  <input placeholder="e.g. Ravi, Priya, Arun" style={FIELD} value={form.team_assigned} onChange={set('team_assigned')} />
                </div>

                {/* Equipment */}
                <div>
                  <label className="block text-[10px] font-bold uppercase tracking-[0.15em] mb-1.5" style={{ color: "#1b365d" }}>Equipment Used</label>
                ...
                <div className="grid grid-cols-2 gap-3">
                  <div>
                    <label className="block text-[10px] font-bold uppercase tracking-[0.15em] mb-1.5" style={{ color: "#1b365d" }}>Travel Expense (₹)</label>
                    <input type="number" min="0" step="1" placeholder="0" style={FIELD}
                      value={form.travel_expense} onChange={set('travel_expense')} />
                  </div>
                  <div>
                    <label className="block text-[10px] font-bold uppercase tracking-[0.15em] mb-1.5" style={{ color: "#1b365d" }}>Travel Time</label>
                    ...
                    <p className="text-[10px] mt-1" style={{ color: "#1b365d" }}>Internal only · not billed</p>
                  </div>
                </div>
