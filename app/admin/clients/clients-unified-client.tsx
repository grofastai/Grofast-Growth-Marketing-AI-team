      ...
      <div style={{ display: 'flex', alignItems: 'center', gap: 10 }}>
        ...
        <div style={{ flex: 1, minWidth: 0 }}>
          ...
          <div style={{ display: 'flex', alignItems: 'center', gap: 6, marginTop: 2, flexWrap: 'wrap' }}>
            {c.industry && <span style={{ fontSize: 10, color: "#000000" }}>{c.industry}</span>}
            {c.package_name && (
              <span style={{
  ...
  return (
    <div style={{
      ...
      <div style={{
        ...
        {count != null && (
          <span style={{ fontSize: 11, fontWeight: 700, padding: '2px 8px', borderRadius: 99, background: '#F3F4F6', color: "#000000" }}>
            {count}
          </span>
        )}
...
function TH({ children }: { children: string }) {
  return (
    <th style={{ padding: '8px 14px', fontSize: 10, fontWeight: 700, color: "#000000", textAlign: 'left', borderBottom: '1px solid #F3F4F6', textTransform: 'uppercase', letterSpacing: '0.06em' }}>
      {children}
    </th>
  )
}
      ...
      <div className={selectedClientName ? "hidden md:flex flex-col flex-1 md:flex-none w-full md:w-[300px]" : "flex flex-col flex-1 md:flex-none w-full md:w-[300px]"} style={{
        ...
        <div style={{ flex: 1, overflowY: 'auto', padding: '10px 12px', display: 'flex', flexDirection: 'column', gap: 2 }}>
          {filteredInternal.length === 0 && filteredActive.length === 0 && filteredPast.length === 0 && (
            <p style={{ fontSize: 12, color: "#000000", textAlign: 'center', padding: '24px 0' }}>No clients found</p>
          )}
                  ...
                  return (
                    <button key={vc.id} onClick={() => selectClient(vc.id)} style={{
                      ...
                      <div style={{ display: 'flex', alignItems: 'center', gap: 8 }}>
                        ...
                        <div style={{ flex: 1, minWidth: 0 }}>
                          <p style={{ fontSize: 12, fontWeight: 800, color: isSel ? vc.color : "#000000", margin: 0, fontFamily: 'var(--font-jakarta)' }}>
                            {vc.label}
                          </p>
                          <p style={{ fontSize: 10, color: "#000000", margin: '1px 0 0' }}>{vc.sub}</p>
                        </div>
                      </div>
                    </button>
                  )
          ...
          {filteredInternal.length > 0 && (
            <>
              <p style={{ fontSize: 9, fontWeight: 800, color: "#000000", textTransform: 'uppercase', letterSpacing: '0.1em', margin: '6px 4px 4px' }}>
      ...
      <div className={!selectedClientName ? "hidden md:flex flex-col" : "flex flex-col"} style={{ flex: 1, overflowY: 'auto' }}>
        ...
        {!selectedClientName && (
          <div style={{ flex: 1, display: 'flex', alignItems: 'center', justifyContent: 'center', flexDirection: 'column', gap: 12, minHeight: '60vh' }}>
            ...
            <p style={{ fontSize: 15, fontWeight: 700, color: "#000000", margin: 0 }}>Select a client</p>
            <p style={{ fontSize: 13, color: "#000000", margin: 0 }}>Pick a client from the left to see their deliverables and costs</p>
          </div>
        )}
          ...
          <div className="px-4 md:px-7" style={{ paddingTop: 24, paddingBottom: 48, display: 'flex', flexDirection: 'column', gap: 20 }}>
            ...
            <div style={{
              ...
              {([
                ...
                <button key={key} onClick={() => setQuick(key)} style={{
                  ...
                  color: activeQuick === key ? '#DE1A1A' : '#000000',
                }}>
                  {label}
                </button>
              ))}
              ...
              {mode === 'custom' ? (
                ...
                <div style={{ display: 'flex', alignItems: 'center', gap: 8, flexWrap: 'wrap' }}>
                  ...
                  />
                  <span style={{ fontSize: 12, fontWeight: 700, color: "#000000" }}>to</span>
                  <input
                    type="date"
                    value={dateTo}
                    min={dateFrom}
            ...
            {deliverables && !hasData && (
              <div style={{
                ...
                <p style={{ fontSize: 14, fontWeight: 700, color: "#000000", margin: '0 0 6px' }}>
                  No work logged for this period
                </p>
                <p style={{ fontSize: 12, color: "#000000", margin: '0 0 16px' }}>
                ...
                <div style={{ display: 'inline-flex', gap: 8, flexWrap: 'wrap', justifyContent: 'center' }}>
                  <p style={{ fontSize: 11, color: "#000000", margin: 0, background: '#F9FAFB', padding: '6px 14px', borderRadius: 8, border: '1px solid #E5E7EB' }}>
                  ...
                  <tbody>
                    {deliverables.shoots.map((s, i) => (
                      <tr key={i} style={{ borderBottom: '1px solid #F9FAFB' }}>
                        <td style={{ padding: '10px 14px', fontSize: 12, color: "#000000" }}>{fmtDate(s.date)}</td>
                        {showClient && <td style={{ padding: '10px 14px', fontSize: 11, fontWeight: 700, color: '#6366F1' }}>{s.clientName}</td>}
                        <td style={{ padding: '10px 14px', fontSize: 12, fontWeight: 600, color: "#000000" }}>{s.memberName}</td>
                        <td style={{ padding: '10px 14px', fontSize: 12, color: "#000000" }}>{s.title}</td>
                    ...
                    <tbody>
                      {allVideos.map((v, i) => (
                        <tr key={i} style={{ borderBottom: '1px solid #F9FAFB', background: v.isRework ? 'rgba(245,158,11,0.04)' : undefined }}>
                          <td style={{ padding: '10px 14px', fontSize: 12, color: "#000000" }}>{fmtDate(v.date)}</td>
                          {showClient && <td style={{ padding: '10px 14px', fontSize: 11, fontWeight: 700, color: '#6366F1' }}>{v.clientName}</td>}
                          <td style={{ padding: '10px 14px', fontSize: 12, fontWeight: 600, color: "#000000" }}>{v.memberName}</td>
                          <td style={{ padding: '10px 14px', fontSize: 12, color: "#000000" }}>
                    ...
                    <tbody>
                      {entries.map((o, i) => (
                        <tr key={i} style={{ borderBottom: '1px solid #F9FAFB', background: o.isRework ? 'rgba(245,158,11,0.04)' : undefined }}>
                          <td style={{ padding: '10px 14px', fontSize: 12, color: "#000000" }}>{fmtDate(o.date)}</td>
                          {showClient && <td style={{ padding: '10px 14px', fontSize: 11, fontWeight: 700, color: '#6366F1' }}>{o.clientName}</td>}
                          <td style={{ padding: '10px 14px', fontSize: 12, fontWeight: 600, color: "#000000" }}>{o.memberName}</td>
                          <td style={{ padding: '10px 14px', fontSize: 12, color: "#000000" }}>
