...
import { Loader2, Plus, X, Bell, Pin, Trash2, Search, Megaphone, Users } from "lucide-react"
  ...
  return (
    <div style={{ marginBottom: 14 }}>
      <div style={{ display: "flex", justifyContent: "space-between", marginBottom: 5 }}>
        <span style={{ fontSize: 12, color: "#6B1D3A", fontWeight: 600 }}>{label}</span>
  ...
  return (
    <div style={{ padding: "24px 28px", maxWidth: 1400, margin: "0 auto" }}>
      ...
      <div style={{ marginBottom: 22 }}>
        <PageHero
          title="Announcements"
          subtitle="Manage and broadcast team communications"
          chips={[
      ...
      <div style={{ display: "flex", justifyContent: "flex-end", alignItems: "center", marginBottom: 24, flexWrap: "wrap", gap: 12 }}>
        <div className="flex flex-wrap gap-3 items-center">
          <button
            ...
            style={{ display: "flex", alignItems: "center", gap: 6, padding: "9px 16px", borderRadius: 12, fontSize: 13, fontWeight: 600, background: "#fff", border: "1.5px solid #E5E7EB", color: "#6B1D3A", cursor: "pointer" }}
          >
        ...
        <div>
          ...
          <div className="flex flex-col sm:flex-row" style={{ borderRadius: 20, overflow: "hidden", border: "1.5px solid #E5E7EB", background: "#fff", marginBottom: 20, minHeight: 180 }}>
            ...
            <div style={{ flex: 1, display: "flex", flexDirection: "column", alignItems: "center", justifyContent: "center", padding: "24px 20px", textAlign: "center", gap: 12 }}>
              ...
              <div>
                ...
                <p style={{ fontSize: 13, color: "#6B1D3A", margin: 0, lineHeight: 1.5 }}>
          ...
          <div style={{ display: "flex", alignItems: "center", justifyContent: "space-between", marginBottom: 16, gap: 12, flexWrap: "wrap" }}>
              ...
              {FILTERS.map((f) => (
                <button
                  ...
                  style={{
                    ...
                    color: activeFilter === f ? "#fff" : "#6B1D3A",
                    border: "none",
                    cursor: "pointer",
            ...
            <div style={{ position: "relative" }}>
              <Search size={14} style={{ position: "absolute", left: 10, top: "50%", transform: "translateY(-50%)", color: "#6B1D3A" }} />
              <input
                value={search}
          ...
          {announcements.length > 0 && filtered.length === 0 ? (
            <div style={{ textAlign: "center", padding: "48px 0", borderRadius: 16, background: "#FAFAFA", border: "1px solid #E5E7EB" }}>
              <Megaphone size={36} style={{ color: "#6B1D3A", marginBottom: 12 }} />
              <p style={{ fontSize: 14, fontWeight: 600, color: "#6B1D3A", margin: 0 }}>No announcements found</p>
              <p style={{ fontSize: 12, color: "#6B1D3A", marginTop: 4 }}>Try a different filter or create a new one.</p>
            </div>
          ) : filtered.length > 0 ? (
                  ...
                  >
                    ...
                    <div style={{ width: 40, height: 40, borderRadius: 12, background: ann.pinned ? "rgba(229,57,53,0.1)" : "#F3F4F6", display: "flex", alignItems: "center", justifyContent: "center", flexShrink: 0 }}>
                      {ann.pinned
                        ...
                        : <Megaphone size={16} style={{ color: "#6B1D3A" }} />}
                    </div>
                    ...
                    <div style={{ flex: 1, minWidth: 0 }}>
                      ...
                      <p style={{ fontSize: 12, color: "#6B1D3A", margin: "0 0 6px", overflow: "hidden", textOverflow: "ellipsis", whiteSpace: "nowrap", maxWidth: 440 }}>
                        {ann.message}
                      </p>
                      <div style={{ display: "flex", alignItems: "center", gap: 10 }}>
                        <span style={{ fontSize: 11, color: "#6B1D3A" }}>By {creator?.name ?? "Admin"}</span>
                        <span style={{ fontSize: 11, color: "#6B1D3A" }}>·</span>
                        <span style={{ fontSize: 11, color: "#6B1D3A" }}>{formatDate(ann.created_at)}</span>
                        <span style={{ fontSize: 11, color: "#6B1D3A" }}>·</span>
                        <span style={{ fontSize: 11, color: "#6B1D3A" }}>{timeAgo(ann.created_at)}</span>
                      </div>
                    </div>
                    ...
                    <div style={{ display: "flex", alignItems: "center", gap: 12, flexShrink: 0 }}>
                      <div style={{ display: "flex", alignItems: "center", gap: 12 }}>
                        <span style={{ fontSize: 12, color: "#6B1D3A", display: "flex", alignItems: "center", gap: 3 }}>
                      ...
                      <div style={{ display: "flex", alignItems: "center", gap: 4 }}>
                        ...
                        >
                          <Pin size={13} style={{ color: ann.pinned ? "#E53935" : "#6B1D3A" }} />
                        </button>
                        <button
        ...
        <div style={{ display: "flex", flexDirection: "column", gap: 16 }}>
          ...
          <div style={{ borderRadius: 20, padding: 20, background: "#fff", border: "1.5px solid #E5E7EB", boxShadow: "0 2px 12px rgba(0,0,0,0.04)" }}>
            <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", marginBottom: 14 }}>
              ...
              <Bell size={14} style={{ color: "#6B1D3A" }} />
            </div>
            {recentNotifications.length === 0 ? (
              <p style={{ fontSize: 12, color: "#6B1D3A", textAlign: "center", padding: "16px 0" }}>No notifications yet</p>
            ) : (
                  ...
                  return (
                    <div key={ann.id} style={{ display: "flex", gap: 10, alignItems: "flex-start" }}>
                      ...
                      <div style={{ flex: 1, minWidth: 0 }}>
                        ...
                        <p style={{ fontSize: 11, color: "#6B1D3A", margin: 0 }}>{timeAgo(ann.created_at)}</p>
                      </div>
                    </div>
                  )
                })}
          ...
          <div style={{ borderRadius: 20, padding: 20, background: "#fff", border: "1.5px solid #E5E7EB", boxShadow: "0 2px 12px rgba(0,0,0,0.04)" }}>
            ...
            <p style={{ fontSize: 11, color: "#6B1D3A", margin: "0 0 16px" }}>Estimated engagement metrics</p>
          ...
          <div style={{ width: "100%", maxWidth: 420, borderRadius: 20, padding: 24, background: "#fff", border: "1px solid #E5E7EB" }}>
            <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", marginBottom: 20 }}>
              ...
              <button onClick={() => { setShowPush(false); setPushResult(null) }} style={{ background: "none", border: "none", cursor: "pointer" }}>
                <X size={18} style={{ color: "#6B1D3A" }} />
              </button>
            </div>
            <div style={{ display: "flex", flexDirection: "column", gap: 14 }}>
              <div>
                <label style={{ fontSize: 10, fontWeight: 700, textTransform: "uppercase", letterSpacing: "0.1em", color: "#6B1D3A", display: "block", marginBottom: 6 }}>Title</label>
              ...
              <div>
                <label style={{ fontSize: 10, fontWeight: 700, textTransform: "uppercase", letterSpacing: "0.1em", color: "#6B1D3A", display: "block", marginBottom: 6 }}>Message</label>
              ...
              <div style={{ display: "flex", gap: 10 }}>
                <button onClick={() => { setShowPush(false); setPushResult(null) }}
                  style={{ flex: 1, padding: "10px", borderRadius: 12, fontSize: 13, fontWeight: 600, background: "#F9FAFB", border: "1.5px solid #E5E7EB", color: "#6B1D3A", cursor: "pointer" }}>
                  Cancel
                </button>
        ...
        <div style={{ position: "fixed", inset: 0, zIndex: 50, display: "flex", alignItems: "center", justifyContent: "center", background: "rgba(0,0,0,0.6)", backdropFilter: "blur(4px)" }}>
          <div style={{ width: "100%", maxWidth: 480, borderRadius: 20, padding: 24, background: "#fff", border: "1px solid #E5E7EB" }}>
            ...
            <form action={(fd) => {
              ...
              <div>
                <label style={{ fontSize: 11, fontWeight: 700, color: "#6B1D3A", display: "block", marginBottom: 6 }}>Category</label>
              ...
              <label style={{ display: "flex", alignItems: "center", gap: 10, cursor: "pointer" }}>
                ...
                <span style={{ fontSize: 13, color: "#6B1D3A" }}>Pin this announcement</span>
              </label>
