      ...
      <svg viewBox="0 0 140 78" style={{ width:"100%", maxWidth:180 }}>
        <defs>
          <linearGradient id="gg" x1="0" y1="0" x2="1" y2="0">
            <stop offset="0%" stopColor="rgba(222,26,26,0.5)"/>
            <stop offset="100%" stopColor={c}/>
          </linearGradient>
        </defs>
        <path d={arc} fill="none" stroke="rgba(255,255,255,0.07)" strokeWidth="12" strokeLinecap="round"/>
        ...
        <text x={cx} y={cy - 8} textAnchor="middle" fontSize="24" fontWeight="900" fill="#FFFFFF">{score}</text>
        <text x={cx} y={cy + 10} textAnchor="middle" fontSize="11" fill={c} fontWeight="700">{lbl}</text>
        <text x={cx - r + 4} y={cy + 20} textAnchor="middle" fontSize="9" fill="#444">0</text>
        <text x={cx + r - 4} y={cy + 20} textAnchor="middle" fontSize="9" fill="#444">100</text>
      </svg>
    </div>
  )
}
...
export default function DailyUpdateForm({
  ...
  if (!isMediaTeam) {
    return (
      <div style={{ background:"#0D0D0D", minHeight:"100vh", padding:"20px 24px 40px", display:"flex", alignItems:"center", justifyContent:"center" }}>
        <div style={{ textAlign:"center" }}>
          <BookOpen size={48} style={{ color:"#333", marginBottom:16 }} />
          <p style={{ fontSize:16, fontWeight:700, color:"#FFFFFF", margin:"0 0 8px" }}>Daily updates for your team</p>
          <p style={{ fontSize:13, color:"#555", margin:0 }}>Coming soon. Contact your admin for details.</p>
        </div>
      </div>
    )
  }
    ...
    return (
      <div style={{ display:"flex", alignItems:"center", justifyContent:"space-between", marginBottom:16 }}>
        <div style={{ display:"flex", alignItems:"center", gap:10 }}>
          ...
          <div>
            <p style={{ fontSize:14, fontWeight:800, color:"#FFFFFF", margin:0 }}>{label}</p>
            <p style={{ fontSize:10, color:"#555", margin:0 }}>{count} entr{count !== 1 ? "ies" : "y"} today</p>
          </div>
        </div>
        <button style={{ background:"none", border:"none", cursor:"pointer", color:"#444", padding:4, borderRadius:6, display:"flex", alignItems:"center" }}>
  ...
  // ── Main render ────────────────────────────────────────────────────────────
  return (
    <div style={{ background:"#0D0D0D", minHeight:"100vh", padding:"20px 24px 40px" }}>

      {/* ── PAGE HEADER ─────────────────────────────────────────────────────── */}
      <div style={{ display:"flex", alignItems:"center", justifyContent:"space-between", marginBottom:22, flexWrap:"wrap", gap:12 }}>
        <div>
          <h1 style={{ fontSize:28, fontWeight:900, color:"#FFFFFF", fontFamily:"var(--font-jakarta)", margin:"0 0 3px" }}>
            Daily <span style={{ color:"#DE1A1A" }}>Update</span>
          </h1>
          <p style={{ fontSize:12, color:"#555", margin:0 }}>{dateLabel}</p>
        </div>

        {/* Greeting card */}
        <div style={{ display:"flex", alignItems:"center", gap:0, borderRadius:16, overflow:"hidden", background:"#1A1A1A", border:"1px solid rgba(255,255,255,0.08)", boxShadow:"0 4px 20px rgba(0,0,0,0.4)" }}>
          ...
          <div style={{ padding:"10px 16px" }}>
            <p style={{ fontSize:13, fontWeight:800, color:"#FFFFFF", margin:"0 0 2px", fontFamily:"var(--font-jakarta)" }}>{greeting}, {firstName}! 👋</p>
            <p style={{ fontSize:11, color:"#666", margin:0 }}>Log your media work for today.</p>
          </div>
        </div>
      </div>
      ...
      <div style={{ display:"grid", gridTemplateColumns:"1fr 280px", gap:18, alignItems:"start" }}>
        ...
        <div style={{ display:"flex", flexDirection:"column", gap:16 }}>
          ...
          {/* Tab selector */}
          <div style={{ display:"flex", gap:6, background:"#1A1A1A", borderRadius:14, padding:5, border:"1px solid rgba(255,255,255,0.06)", width:"fit-content" }}>
          ...
          {tab === "working" && (<>
            ...
            {/* ── SHOOTS SECTION ────────────────────────────────────────────── */}
            <div style={{ background:"#141414", borderRadius:20, border:"1px solid rgba(255,255,255,0.07)", padding:"20px 22px", boxShadow:"0 4px 24px rgba(0,0,0,0.35)" }}>
              ...
              {shoots.length === 0 ? (
                ...
                <div style={{ display:"flex", flexDirection:"column", gap:12 }}>
                  {shoots.map((s, i) => (
                    <div key={s.id} style={{ background:"#1C1C1C", borderRadius:14, border:"1px solid rgba(255,255,255,0.06)", padding:"14px 16px" }}>
                      <div style={{ display:"flex", alignItems:"center", justifyContent:"space-between", marginBottom:12 }}>
                        ...
                        <button onClick={() => removeShoot(s.id)}
                          style={{ width:26, height:26, borderRadius:8, background:"rgba(239,68,68,0.1)", border:"none", cursor:"pointer", display:"flex", alignItems:"center", justifyContent:"center" }}>
                          <Trash2 size={12} style={{ color:"#EF4444" }} />
                        </button>
                      </div>
                      <div style={{ display:"grid", gridTemplateColumns:"1fr 1fr", gap:10, marginBottom:10 }}>
                        <div>
                          <label style={{ display:"block", fontSize:10, fontWeight:700, color:"#666", textTransform:"uppercase", letterSpacing:"0.1em", marginBottom:5 }}>Client / Project *</label>
                          {projects.length > 0 ? (
                            <div style={{ position:"relative" }}>
                              ...
                              <ChevronDown size={11} style={{ position:"absolute", right:10, top:"50%", transform:"translateY(-50%)", color:"#555", pointerEvents:"none" }} />
                            </div>
                          ) : (
                        ...
                        <div>
                          <label style={{ display:"block", fontSize:10, fontWeight:700, color:"#666", textTransform:"uppercase", letterSpacing:"0.1em", marginBottom:5 }}>Shoot Type / Title *</label>
                      ...
                      <div style={{ display:"grid", gridTemplateColumns:"1fr 1fr 100px", gap:10, marginBottom:10 }}>
                        <div>
                          <label style={{ display:"block", fontSize:10, fontWeight:700, color:"#666", textTransform:"uppercase", letterSpacing:"0.1em", marginBottom:5 }}>Start Time</label>
                          <div style={{ position:"relative" }}>
                        ...
                        <div>
                          <label style={{ display:"block", fontSize:10, fontWeight:700, color:"#666", textTransform:"uppercase", letterSpacing:"0.1em", marginBottom:5 }}>End Time</label>
                          <div style={{ position:"relative" }}>
                        ...
                        <div>
                          <label style={{ display:"block", fontSize:10, fontWeight:700, color:"#666", textTransform:"uppercase", letterSpacing:"0.1em", marginBottom:5 }}>Duration</label>
                          <DurationPicker value={s.durationHours} onChange={v => patchShoot(s.id, { durationHours: v })} />
                        </div>
                      </div>
                      <div style={{ display:"grid", gridTemplateColumns:"1fr auto", gap:10, alignItems:"start" }}>
                        <div>
                          <label style={{ display:"block", fontSize:10, fontWeight:700, color:"#666", textTransform:"uppercase", letterSpacing:"0.1em", marginBottom:5 }}>Notes</label>
                        ...
                        <div style={{ paddingTop:24 }}>
                          <button onClick={() => patchShoot(s.id, { videoUploaded: !s.videoUploaded })}
                            style={{ display:"flex", alignItems:"center", gap:6, padding:"9px 14px", borderRadius:10, border:"1.5px solid", cursor:"pointer", fontSize:11, fontWeight:700, transition:"all 0.15s",
                              background:  s.videoUploaded ? "rgba(34,197,94,0.1)"  : "#1C1C1C",
                              borderColor: s.videoUploaded ? "rgba(34,197,94,0.4)"  : "rgba(255,255,255,0.1)",
                              color:       s.videoUploaded ? "#22C55E" : "#555",
                            }}>
                            <Upload size={12} />
              ...
              {shoots.length > 0 && (
                <button onClick={addShoot}
                  style={{ display:"flex", alignItems:"center", gap:7, marginTop:12, padding:"9px 16px", borderRadius:10, background:"rgba(239,68,68,0.07)", border:"1.5px dashed rgba(239,68,68,0.25)", color:"#EF4444", fontSize:12, fontWeight:700, cursor:"pointer" }}>
            ...
            <div style={{ background:"#141414", borderRadius:20, border:"1px solid rgba(255,255,255,0.07)", padding:"20px 22px", boxShadow:"0 4px 24px rgba(0,0,0,0.35)" }}>
              ...
              {edits.length === 0 ? (
                ...
                <div style={{ display:"flex", flexDirection:"column", gap:12 }}>
                  {edits.map((e, i) => (
                    <div key={e.id} style={{ background:"#1C1C1C", borderRadius:14, border:"1px solid rgba(255,255,255,0.06)", padding:"14px 16px" }}>
                      <div style={{ display:"flex", alignItems:"center", justifyContent:"space-between", marginBottom:12 }}>
                        ...
                        <button onClick={() => removeEdit(e.id)}
                          style={{ width:26, height:26, borderRadius:8, background:"rgba(99,102,241,0.1)", border:"none", cursor:"pointer", display:"flex", alignItems:"center", justifyContent:"center" }}>
                      ...
                      <div style={{ display:"grid", gridTemplateColumns:"1fr 1fr 100px", gap:10, marginBottom:10 }}>
                        ...
                        <div>
                          <label style={{ display:"block", fontSize:10, fontWeight:700, color:"#666", textTransform:"uppercase", letterSpacing:"0.1em", marginBottom:5 }}>Video / Project Title *</label>
                      ...
                      <div style={{ display:"grid", gridTemplateColumns:"1fr 1fr", gap:10 }}>
                        ...
                        <div>
                          <label style={{ display:"block", fontSize:10, fontWeight:700, color:"#666", textTransform:"uppercase", letterSpacing:"0.1em", marginBottom:5 }}>
              ...
              {edits.length > 0 && (
                <button onClick={addEdit}
                  style={{ display:"flex", alignItems:"center", gap:7, marginTop:12, padding:"9px 16px", borderRadius:10, background:"rgba(99,102,241,0.07)", border:"1.5px dashed rgba(99,102,241,0.25)", color:"#6366F1", fontSize:12, fontWeight:700, cursor:"pointer" }}>
          ...
          {tab === "learning" && (
            <div style={{ background:"#141414", borderRadius:20, border:"1px solid rgba(255,255,255,0.07)", padding:"20px 22px", boxShadow:"0 4px 24px rgba(0,0,0,0.35)" }}>
              ...
              <div style={{ display:"flex", flexDirection:"column", gap:14 }}>
                <div>
                  <label style={{ display:"block", fontSize:10, fontWeight:700, color:"#666", textTransform:"uppercase", letterSpacing:"0.1em", marginBottom:6 }}>Topic / Course *</label>
                ...
                <div style={{ display:"grid", gridTemplateColumns:"160px 1fr", gap:14 }}>
                  <div>
                    <label style={{ display:"block", fontSize:10, fontWeight:700, color:"#666", textTransform:"uppercase", letterSpacing:"0.1em", marginBottom:6 }}>Hours Spent *</label>
                    <DurationPicker value={learningHours} onChange={setLearningHours} />
                  </div>
                  <div>
                    <label style={{ display:"block", fontSize:10, fontWeight:700, color:"#666", textTransform:"uppercase", letterSpacing:"0.1em", marginBottom:6 }}>Notes</label>
          ...
          {/* ── SUBMIT BAR ────────────────────────────────────────────────────── */}
          <div style={{ background:"#141414", borderRadius:16, border:"1px solid rgba(255,255,255,0.07)", padding:"14px 18px", display:"flex", alignItems:"center", justifyContent:"space-between", gap:12, boxShadow:"0 4px 24px rgba(0,0,0,0.35)" }}>
            <div>
        ...
        <div style={{ display:"flex", flexDirection:"column", gap:14 }}>
          ...
          {/* Today's Overview */}
          <div style={{ background:"#141414", borderRadius:20, border:"1px solid rgba(255,255,255,0.07)", overflow:"hidden", boxShadow:"0 4px 24px rgba(0,0,0,0.35)" }}>
            <div style={{ position:"relative", height:150 }}>
              <Image src="/brand/daily-boy.png" alt="" fill style={{ objectFit:"cover", objectPosition:"center top" }} />
              <div style={{ position:"absolute", inset:0, background:"linear-gradient(to bottom, transparent 40%, #141414 100%)" }} />
            </div>
            <div style={{ padding:"14px 16px" }}>
              <p style={{ fontSize:13, fontWeight:800, color:"#FFFFFF", margin:"0 0 14px", display:"flex", alignItems:"center", gap:7 }}>
                <BarChart2 size={13} style={{ color:"#DE1A1A" }} /> Today&apos;s Overview
              </p>
              <div style={{ display:"flex", flexDirection:"column", gap:10 }}>
                {([
                  { icon: <Clock  size={12} style={{ color:"#888" }}    />, label:"Hours Logged", text: totalHours > 0 ? `${totalHours}h` : "—", color: totalHours >= 8 ? "#22C55E" : "#FFFFFF", badge: null },
                  { icon: <Camera size={12} style={{ color:"#EF4444" }} />, label:"Shoots",        text: String(shoots.length),                  color: shoots.length  > 0 ? "#EF4444" : "#555",   badge: null },
                  { icon: <Film   size={12} style={{ color:"#6366F1" }} />, label:"Edits",          text: String(edits.length),                   color: edits.length   > 0 ? "#6366F1" : "#555",   badge: null },
                  { icon: <Zap    size={12} style={{ color:"#F59E0B" }} />, label:"Status",         text: null, color: "#fff",
          ...
          {/* Productivity Score */}
          <div style={{ background:"#141414", borderRadius:20, border:"1px solid rgba(255,255,255,0.07)", padding:"16px", boxShadow:"0 4px 24px rgba(0,0,0,0.35)" }}>
            <div style={{ display:"flex", alignItems:"center", gap:7, marginBottom:12 }}>
              <BarChart2 size={14} style={{ color:"#DE1A1A" }} />
              <p style={{ fontSize:13, fontWeight:800, color:"#FFFFFF", margin:0 }}>Productivity Score</p>
            </div>
          ...
          {/* Consistency card */}
          <div style={{ background:"#FFFFFF", borderRadius:20, padding:"18px 16px", boxShadow:"0 4px 24px rgba(0,0,0,0.2)", display:"flex", alignItems:"flex-start", gap:14 }}>
          ...
          {(totalShootHours > 0 || totalEditHours > 0) && (
            <div style={{ background:"#141414", borderRadius:20, border:"1px solid rgba(255,255,255,0.07)", padding:"16px", boxShadow:"0 4px 24px rgba(0,0,0,0.35)" }}>
              <div style={{ display:"flex", alignItems:"center", gap:7, marginBottom:14 }}>
                ...
                <p style={{ fontSize:13, fontWeight:800, color:"#FFFFFF", margin:0 }}>Hours Breakdown</p>
              </div>
              {[
