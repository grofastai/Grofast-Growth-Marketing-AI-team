...
export function AttendanceDonut({ present, notLogged, absent, total }: DonutProps) {
  const slices = [
    ...
    { name: "Absent",       value: absent,     color: "#000000" },
  ]
    ...
    <div className="flex flex-wrap items-center gap-5">
      <div style={{ position: "relative", width: 130, height: 130, flexShrink: 0 }}>
        ...
        <div style={{ position: "absolute", inset: 0, display: "flex", flexDirection: "column", alignItems: "center", justifyContent: "center", pointerEvents: "none" }}>
          ...
          <span style={{ fontSize: 9, color: "#000000", fontWeight: 700, marginTop: 1 }}>Total</span>
        </div>
      </div>
        ...
        {slices.map(s => (
          <div key={s.name} style={{ display: "flex", alignItems: "center", justifyContent: "space-between" }}>
            <div style={{ display: "flex", alignItems: "center", gap: 7 }}>
              ...
              <span style={{ fontSize: 12, color: "#000000", fontWeight: 600 }}>{s.name}</span>
            </div>
    ...
    <div className="w-full" style={{ height: 130 }}>
      <ResponsiveContainer width="100%" height="100%">
        <AreaChart data={data} margin={{ top: 5, right: 5, left: -20, bottom: 0 }}>
          ...
          <XAxis dataKey="day" tick={{ fontSize: 10, fill: "#000000" }} axisLine={false} tickLine={false} />
          <YAxis tick={{ fontSize: 10, fill: "#000000" }} axisLine={false} tickLine={false} allowDecimals={false} />
