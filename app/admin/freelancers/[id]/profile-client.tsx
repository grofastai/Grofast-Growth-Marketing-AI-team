...
function Label({ text }: { text: string }) {
  return <label style={{ fontSize: 11, fontWeight: 700, color: "#7F011F", textTransform: "uppercase", letterSpacing: "0.05em" }}>{text}</label>
}
    ...
    <div style={{ background: "#F5F6FA", minHeight: "100vh" }} className="p-4 lg:p-6 space-y-4">
      ...
      <div className="flex items-center gap-3 flex-wrap">
        <Link href="/admin/freelancers"
          ...
          <ArrowLeft size={16} style={{ color: "#7F011F" }} />
        </Link>
        <div style={{ flex: 1 }}>
        ...
        <span style={{
          ...
          color: freelancer.status === "active" ? "#16A34A" : "#7F011F",
        }}>
        ...
        {TABS.map(t => {
          ...
          return (
            <button key={t.key} onClick={() => setActiveTab(t.key)}
              ...
              style={active ? { background: "#DE1A1A", color: "#FFF" } : { color: "#7F011F" }}>
    ...
    <div className="space-y-4">
      <div className="grid grid-cols-2 md:grid-cols-3 lg:grid-cols-5 gap-3">
        {statCards.map(s => (
          <div key={s.label} style={{ ...CARD, padding: 16 }}>
            ...
            <p style={{ fontSize: 11, color: "#7F011F", margin: "4px 0 0", fontWeight: 600 }}>{s.label}</p>
          </div>
        ))}
      </div>
      ...
      <div style={{ ...CARD, padding: 20 }}>
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
          {details.map(row => (
            <div key={row.label}>
              <p style={{ fontSize: 10, fontWeight: 700, color: "#7F011F", textTransform: "uppercase", margin: 0 }}>{row.label}</p>
    ...
    <div ref={ref} style={{ position: "relative" }}>
      {!isManual && (
        <button type="button" onClick={() => setOpen(o => !o)} style={btnStyle}>
          <span style={{ color: value ? "#111827" : "#7F011F" }}>{value || "Select client…"}</span>
          <ChevronDown size={13} style={{ color: "#7F011F", flexShrink: 0 }} />
        </button>
      )}
      {isManual && (
        <div style={{ display: "flex", gap: 6, marginTop: 4 }}>
          ...
          <button type="button" onClick={() => { onChange(""); setView("main"); setOpen(true) }}
            style={{ padding: "0 10px", borderRadius: 8, border: "1px solid #E5E7EB", background: "#FFF", color: "#7F011F", cursor: "pointer", fontSize: 12 }}>✕</button>
        </div>
      )}
      {open && !isManual && (
        <div style={{ position: "absolute", top: "100%", left: 0, right: 0, marginTop: 4, background: "#FFF", border: "1px solid #E5E7EB", borderRadius: 12, boxShadow: "0 8px 24px rgba(0,0,0,0.12)", zIndex: 999, maxHeight: 240, overflowY: "auto" }}>
          {view === "main" && <>
            ...
            {activeClientNames.map(n => (
              <button key={n} type="button" onClick={() => pick(n)} style={{ ...rowStyle, color: "#7F011F" }}
          ...
          {view === "past" && <>
            <button type="button" onClick={() => setView("main")}
              style={{ ...rowStyle, color: "#7F011F", fontSize: 12, borderBottom: "1px solid #F3F4F6" }}
              onMouseEnter={e => (e.currentTarget.style.background = "#F9FAFB")}
              onMouseLeave={e => (e.currentTarget.style.background = "none")}>
              ← Back
            </button>
    ...
    <div className="space-y-4">
        ...
        <div style={{ position: "fixed", inset: 0, background: "rgba(0,0,0,0.4)", zIndex: 50, display: "flex", alignItems: "center", justifyContent: "center", padding: 16 }}>
          <div style={{ background: "#FFF", borderRadius: 20, padding: 24, width: "100%", maxWidth: 480, maxHeight: "90vh", overflowY: "auto" }}>
            <div className="flex items-center justify-between mb-5">
              ...
              <button onClick={() => setShowAdd(false)}><X size={18} style={{ color: "#7F011F" }} /></button>
            </div>
            <div className="space-y-3">
              ...
              <div>
                ...
                {total > 0 && !form.manual_amount && (
                  <p style={{ fontSize: 11, color: "#7F011F", marginTop: 4 }}>Auto-calculated: ₹{total}. Override above if different.</p>
                )}
              </div>
            ...
            <div className="flex gap-2 mt-5">
              <button onClick={() => setShowAdd(false)} style={{ flex: 1, padding: 11, borderRadius: 10, border: "1px solid #E5E7EB", background: "#FFF", fontSize: 13, fontWeight: 700, color: "#7F011F", cursor: "pointer" }}>Cancel</button>
        ...
        <div style={{ position: "fixed", inset: 0, background: "rgba(0,0,0,0.4)", zIndex: 50, display: "flex", alignItems: "center", justifyContent: "center", padding: 16 }}>
          <div style={{ background: "#FFF", borderRadius: 20, padding: 24, width: "100%", maxWidth: 400 }}>
            ...
            <div className="flex gap-2 mt-4">
              <button onClick={() => { setRejectId(null); setRejectReason("") }} style={{ flex: 1, padding: 10, borderRadius: 10, border: "1px solid #E5E7EB", background: "#FFF", fontSize: 13, fontWeight: 700, color: "#7F011F", cursor: "pointer" }}>Cancel</button>
      ...
      {workEntries.length === 0 ? (
        <div style={{ ...CARD, padding: 40, textAlign: "center" }}>
          <p style={{ color: "#7F011F", fontSize: 14 }}>No work entries yet.</p>
        </div>
      ) : (
        <div className="space-y-3">
          {workEntries.map(e => (
            <div key={e.id} style={{ ...CARD, padding: 16 }}>
              <div className="flex items-start justify-between gap-3 flex-wrap">
                <div style={{ flex: 1, minWidth: 0 }}>
                  ...
                  <p style={{ fontSize: 12, color: "#7F011F", margin: "2px 0 0" }}>
                    {e.client_name && `${e.client_name} · `}{fmtDate(e.date)}
                  </p>
                  {e.notes && <p style={{ fontSize: 11, color: "#7F011F", margin: "4px 0 0" }}>{e.notes}</p>}
                ...
                <div className="flex items-center gap-2 flex-wrap flex-shrink-0">
                  ...
                  <span style={{ fontSize: 10, fontWeight: 700, padding: "3px 8px", borderRadius: 6, background: APPROVAL_BG[e.approval_status] ?? "rgba(107,114,128,0.08)", color: APPROVAL_COLOR[e.approval_status] ?? "#7F011F" }}>
...
function PaymentsTab({ freelancer, payments, stats, currentUserRole }: {
  ...
  return (
    <div className="space-y-4">
      <div className="grid grid-cols-2 md:grid-cols-4 gap-3">
        {balanceSummary.map(s => (
          <div key={s.label} style={{ ...CARD, padding: 16 }}>
            ...
            <p style={{ fontSize: 11, color: "#7F011F", margin: "4px 0 0" }}>{s.label}</p>
          </div>
        ))}
      </div>
      ...
      {payments.length === 0 ? (
        <div style={{ ...CARD, padding: 40, textAlign: "center" }}>
          <p style={{ color: "#7F011F", fontSize: 14 }}>No payments recorded yet.</p>
        </div>
      ) : (
        <div style={CARD}>
          {payments.map((p, i) => (
            <div key={p.id} style={{ display: "flex", alignItems: "center", gap: 12, padding: "14px 20px", borderBottom: i < payments.length - 1 ? "1px solid #F3F4F6" : "none" }}>
              ...
              <div style={{ flex: 1 }}>
                <p style={{ fontSize: 14, fontWeight: 700, color: "#111827", margin: 0 }}>
                  {fmt(p.amount)} <span style={{ fontSize: 11, fontWeight: 600, color: "#7F011F" }}>via {METHOD_LABEL[p.payment_method] ?? p.payment_method}</span>
                </p>
                <p style={{ fontSize: 11, color: "#7F011F", margin: "2px 0 0" }}>
                  {fmtDate(p.paid_date)}{p.reference_number ? ` · Ref: ${p.reference_number}` : ""}
                </p>
                {p.notes && <p style={{ fontSize: 11, color: "#7F011F", margin: "2px 0 0" }}>{p.notes}</p>}
              </div>
              {canDelete && (
      ...
      <div className="space-y-3">
        <div>
          ...
          <div style={{ position: "relative" }}>
            ...
            <ChevronDown size={14} style={{ position: "absolute", right: 10, top: "50%", transform: "translateY(-50%)", color: "#7F011F", pointerEvents: "none" }} />
          </div>
        </div>
        <div>
          <Label text="Year" />
        ...
        <p style={{ fontSize: 11, color: "#7F011F", textAlign: "center" }}>
...
function ActivityTab({ activityLogs }: { activityLogs: ActivityRecord[] }) {
  if (activityLogs.length === 0) {
    return (
      <div style={{ ...CARD, padding: 40, textAlign: "center" }}>
        <p style={{ color: "#7F011F", fontSize: 14 }}>No activity yet.</p>
      </div>
    )
  }
      ...
      {activityLogs.map((log, i) => (
        <div key={log.id} style={{ display: "flex", gap: 12, padding: "14px 20px", borderBottom: i < activityLogs.length - 1 ? "1px solid #F3F4F6" : "none" }}>
          ...
          <div style={{ flex: 1 }}>
            ...
            {log.remarks && <p style={{ fontSize: 11, color: "#7F011F", margin: "2px 0 0" }}>{log.remarks}</p>}
            <p style={{ fontSize: 11, color: "#7F011F", margin: "4px 0 0" }}>
