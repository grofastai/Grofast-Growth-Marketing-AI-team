...
- **Auto-confirm collaboration tags for management team** — management (`is_management=true`) doesn't submit their own daily updates, so every collaboration tag on them sits "pending" until they manually confirm/edit/reject it, same flow as a regular member. Ask: auto-confirm immediately at creation time when the tagged collaborator is management, instead of waiting on a manual confirm. Not started.
