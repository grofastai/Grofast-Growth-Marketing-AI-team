...
function ClientExpenseModalBody({ clients, selectedMonth, editing, onClose }: {
  ...
  return (
      <div className="space-y-4">
        <div>
          <label className="text-[11px] font-bold uppercase tracking-wider" style={{ color: "#374151" }}>Client</label>
          ...
          {(["ad", "software", "other"] as const).map(t => (
            <button key={t} onClick={() => setType(t)}
              ...
              style={{
                ...
                color: type === t ? TYPE_COLOR[t] : "#374151",
              }}>
              {TYPE_ICON[t]}
              {t}
            </button>
        ...
        <div className="grid grid-cols-2 gap-3">
          <div>
            <label className="text-[11px] font-bold uppercase tracking-wider" style={{ color: "#374151" }}>Date</label>
            <input type="date" max="2099-12-31" value={date} onChange={e => setDate(e.target.value)}
              className="mt-1 w-full rounded-xl px-3 py-2.5 text-[13px] outline-none"
              style={{ background: "#F9FAFB", border: "1px solid #E5E7EB", color: "#111111" }} />
          </div>
          <div>
            <label className="text-[11px] font-bold uppercase tracking-wider" style={{ color: "#374151" }}>Amount (₹)</label>
        ...
        <div>
          <label className="text-[11px] font-bold uppercase tracking-wider" style={{ color: "#374151" }}>Notes (optional)</label>
...
function CommonExpenseModalBody({ selectedMonth, overheadDivisor, editing, onClose }: {
  ...
  return (
      <div className="space-y-4">
        <div>
          <label className="text-[11px] font-bold uppercase tracking-wider" style={{ color: "#374151" }}>Expense Name</label>
        ...
        {previewShare > 0 && (
          <div className="flex items-center gap-2 px-3 py-2.5 rounded-xl"
            ...
            <span className="text-[12px]" style={{ color: "#374151" }}>
              ...
              <span style={{ color: "#374151" }}> ÷ {overheadDivisor}</span>
            </span>
          </div>
        )}
        <div>
    ...
    <FlatCard className="overflow-hidden">
      ...
      {shoots.length === 0 ? (
        <div className="flex flex-col items-center py-16">
          ...
          <p className="text-[13px]" style={{ color: "#374151" }}>No shoots logged this month</p>
        </div>
      ) : (
          ...
          <table style={{ minWidth: 720 }} className="w-full">
            <thead>
              <tr style={{ background: "#FAFAFA", borderBottom: "1px solid #EDEDED" }}>
                <th className="px-6 py-3 text-left text-[10px] font-bold uppercase tracking-wider whitespace-nowrap" style={{ color: "#374151" }}>Date</th>
                <th className="px-4 py-3 text-left text-[10px] font-bold uppercase tracking-wider whitespace-nowrap" style={{ color: "#374151" }}>Client</th>
                <th className="px-4 py-3 text-left text-[10px] font-bold uppercase tracking-wider whitespace-nowrap" style={{ color: "#374151" }}>Shoot Title</th>
                <th className="px-4 py-3 text-left text-[10px] font-bold uppercase tracking-wider whitespace-nowrap" style={{ color: "#374151" }}>Member</th>
                <th className="px-4 py-3 text-center text-[10px] font-bold uppercase tracking-wider whitespace-nowrap" style={{ color: "#374151" }}>Hrs</th>
                <th className="px-4 py-3 text-right text-[10px] font-bold uppercase tracking-wider whitespace-nowrap" style={{ color: "#374151" }}>Travel ₹</th>
                <th className="px-6 py-3 text-right text-[10px] font-bold uppercase tracking-wider whitespace-nowrap" style={{ color: "#374151" }}>Action</th>
              </tr>
            </thead>
            <tbody>
              {shoots.map((row, i) => {
                ...
                return (
                  <tr key={row.key} style={{ borderBottom: i < shoots.length - 1 ? "1px solid #F5F5F5" : "none" }}>
                    <td className="px-6 py-3 whitespace-nowrap text-[12px] font-semibold" style={{ color: "#374151" }}>{fmtDate(row.date)}</td>
                    ...
                    <td className="px-4 py-3 text-[12px] whitespace-nowrap" style={{ color: "#374151" }}>{row.memberName}</td>
          ...
          {(() => {
            ...
            return (
              <div
                ...
                <p className="text-[11px] mt-3" style={{ color: isTravelActive ? "rgba(255,255,255,0.7)" : "#374151", position: "relative" }}>{MONTHS_SHORT[mo - 1]} {yr}</p>
              </div>
            )
          })()}
          {([
            ...
            return (
              <FlatCard key={s.label} className="p-5 transition-all" onClick={() => setActiveTab(s.key)}
                ...
                <div className="flex items-center gap-2 mb-2" style={{ position: "relative" }}>
                  ...
                  <p className="text-[12px] font-bold" style={{ color: isActive ? "rgba(255,255,255,0.85)" : "#374151" }}>{s.label}</p>
                </div>
                ...
                <p className="text-[10px] mt-1" style={{ color: isActive ? "rgba(255,255,255,0.7)" : "#374151", position: "relative" }}>{pctOfTotal(s.value)}% of total</p>
        ...
        <div className="flex flex-col md:flex-row items-stretch gap-3">
          <FlatCard className="p-5 flex-1 flex items-center gap-4">
            ...
            <div>
              ...
              <p className="text-[12px] mt-1" style={{ color: "#374151" }}>Monitor all your client wise expenses in one place. Analyse, compare and optimise your business spending.</p>
            </div>
          </FlatCard>
          ...
          <FlatCard className="overflow-hidden">
            ...
            {clientExpenses.length === 0 ? (
              <div className="flex flex-col items-center py-12">
                ...
                <p className="text-[13px]" style={{ color: "#374151" }}>No client expenses this month</p>
              </div>
            ) : (
              <div className="overflow-x-auto">
                <table style={{ minWidth: 720 }} className="w-full">
                  <thead>
                    <tr style={{ background: "#FAFAFA", borderBottom: "1px solid #EDEDED" }}>
                      <th className="px-6 py-3 text-left text-[10px] font-bold uppercase tracking-wider whitespace-nowrap" style={{ color: "#374151" }}>Date</th>
                      <th className="px-4 py-3 text-left text-[10px] font-bold uppercase tracking-wider whitespace-nowrap" style={{ color: "#374151" }}>Client</th>
                      <th className="px-4 py-3 text-left text-[10px] font-bold uppercase tracking-wider whitespace-nowrap" style={{ color: "#374151" }}>Type</th>
                      <th className="px-4 py-3 text-left text-[10px] font-bold uppercase tracking-wider whitespace-nowrap" style={{ color: "#374151" }}>Details</th>
                      <th className="px-4 py-3 text-right text-[10px] font-bold uppercase tracking-wider whitespace-nowrap" style={{ color: "#374151" }}>Amount</th>
                      <th className="px-6 py-3 text-right text-[10px] font-bold uppercase tracking-wider whitespace-nowrap" style={{ color: "#374151" }}>Action</th>
                    </tr>
                  </thead>
                  <tbody>
                    {clientExpenses.map((e, i) => (
                      <tr key={e.id} style={{ borderBottom: i < clientExpenses.length - 1 ? "1px solid #F5F5F5" : "none" }}>
                        <td className="px-6 py-3 text-[12px] font-semibold whitespace-nowrap" style={{ color: "#374151" }}>{fmtDate(e.date)}</td>
                        ...
                        <td className="px-6 py-3">
                          <div className="flex items-center justify-end gap-2">
                            <button onClick={() => { setEditClient(e); setModal("client") }} className="opacity-40 hover:opacity-90 transition-opacity">
                              <Pencil size={13} style={{ color: "#374151" }} />
                            </button>
        ...
        {activeTab === "common" && (
          <FlatCard className="overflow-hidden">
            ...
            {commonExpenses.length === 0 ? (
              <div className="flex flex-col items-center py-12">
                <AlertCircle size={28} style={{ color: "#E5E7EB" }} className="mb-2" />
                <p className="text-[13px]" style={{ color: "#374151" }}>No common expenses this month</p>
              </div>
            ) : (
              <div className="overflow-x-auto">
                <table style={{ minWidth: 640 }} className="w-full">
                  <thead>
                    <tr style={{ background: "#FAFAFA", borderBottom: "1px solid #EDEDED" }}>
                      <th className="px-6 py-3 text-left text-[10px] font-bold uppercase tracking-wider whitespace-nowrap" style={{ color: "#374151" }}>Name</th>
                      <th className="px-4 py-3 text-left text-[10px] font-bold uppercase tracking-wider whitespace-nowrap" style={{ color: "#374151" }}>Notes</th>
                      <th className="px-4 py-3 text-right text-[10px] font-bold uppercase tracking-wider whitespace-nowrap" style={{ color: "#374151" }}>Per Client</th>
                      <th className="px-4 py-3 text-right text-[10px] font-bold uppercase tracking-wider whitespace-nowrap" style={{ color: "#374151" }}>Amount</th>
                      <th className="px-6 py-3 text-right text-[10px] font-bold uppercase tracking-wider whitespace-nowrap" style={{ color: "#374151" }}>Action</th>
                    </tr>
                  </thead>
                    ...
                    {commonExpenses.map((e, i) => {
                      ...
                      return (
                        <tr key={e.id} style={{ borderBottom: i < commonExpenses.length - 1 ? "1px solid #F5F5F5" : "none" }}>
                          ...
                          <td className="px-4 py-3 text-[12px]" style={{ color: "#374151" }}>{e.notes || "—"}</td>
                          <td className="px-4 py-3 text-right text-[12px] font-semibold whitespace-nowrap" style={{ color: share > 0 ? "#8B5CF6" : "#374151" }}>
          ...
          <FlatCard className="overflow-hidden">
            ...
            <div className="overflow-x-auto">
              <table style={{ minWidth: 640 }} className="w-full">
                <thead>
                  <tr style={{ background: "#FAFAFA", borderBottom: "1px solid #EDEDED" }}>
                    <th className="px-6 py-3 text-left text-[10px] font-bold uppercase tracking-wider whitespace-nowrap" style={{ color: "#374151" }}>Client / Brand</th>
                ...
                <tbody>
                  {filteredSummaryRows.length === 0 ? (
                    <tr><td colSpan={6} className="px-6 py-10 text-center text-[13px]" style={{ color: "#374151" }}>No clients match this filter</td></tr>
                    ...
                    return (
                      <tr key={row.name} style={{ borderBottom: i < filteredSummaryRows.length - 1 ? "1px solid #F5F5F5" : "none" }}>
                        ...
                        <td className="px-4 py-3 text-right">
                          <div className="text-[13px] font-black" style={{ color: row.empCost > 0 ? "#059669" : "#374151", fontVariantNumeric: "tabular-nums" }}>{fmtRupee(row.empCost)}</div>
                        </td>
                        <td className="px-4 py-3 text-right">
                          <div className="text-[13px] font-black" style={{ color: row.direct > 0 ? "#6366F1" : "#374151", fontVariantNumeric: "tabular-nums" }}>{fmtRupee(row.direct)}</div>
                        </td>
                        <td className="px-4 py-3 text-right">
                        ...
                        <td className="px-6 py-3">
                          <div className="flex items-center justify-end gap-2">
                            ...
                            <button className="w-7 h-7 rounded-lg flex items-center justify-center flex-shrink-0" style={{ color: "#374151" }}>
          ...
          <div className="space-y-5">
            ...
            <div>
              ...
              {viewDetailsEntries.length === 0 ? (
                <p className="text-[12px]" style={{ color: "#374151" }}>No direct expenses logged for this client.</p>
              ) : (
                  ...
                  {viewDetailsEntries.map(e => (
                    <FlatCard key={e.id} className="p-3 flex items-center justify-between">
                      <div>
                        ...
                        <p className="text-[10px]" style={{ color: "#374151" }}>{fmtDate(e.date)}{e.notes ? ` · ${e.notes}` : ""}</p>
                      </div>
