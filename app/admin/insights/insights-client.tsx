            ...
            <tbody>
              {memberUtilization.length === 0 ? (
                ...
                <tr key={m.id} style={{ borderBottom: '1px solid #F9FAFB', background: i % 2 === 0 ? '#fff' : '#FAFBFF' }}>
                  ...
                  {/* Present */}
                  <td style={{ padding: '12px 14px', fontSize: 13, fontWeight: 700, color: "#5D0565", textAlign: 'right' }}>
                    {m.workingDays}
                  </td>
                  {/* Login Hrs */}
            ...
            {memberUtilization.length > 0 && (
              <tfoot>
                <tr style={{ borderTop: '2px solid #EBEDF2', background: '#F9FAFB' }}>
                  <td colSpan={2} style={{ padding: '10px 14px', fontSize: 11, fontWeight: 800, color: "#5D0565" }}>TOTAL / AVG</td>
                  <td style={{ padding: '10px 14px', textAlign: 'right', fontSize: 12, fontWeight: 700, color: "#5D0565" }}>
        ...
        <div style={{ padding: '16px 20px', display: 'flex', flexDirection: 'column', gap: 10 }}>
          {workTotals.map(w => (
            <div key={w.key} style={{ display: 'flex', alignItems: 'center', gap: 12 }}>
              ...
              <span style={{ fontSize: 12, fontWeight: 700, color: "#5D0565", width: 90, flexShrink: 0 }}>{w.label}</span>
              ...
              return (
                <div key={c.name} style={{
                  ...
                  <div style={{ flex: 1, minWidth: 0 }}>
                    <p style={{ fontSize: 11, fontWeight: 700, color: "#5D0565", margin: 0, overflow: 'hidden', textOverflow: 'ellipsis', whiteSpace: 'nowrap' }}>
                      {c.name}
                    </p>
                    ...
                    <div style={{ padding: '0 18px 14px 18px', display: 'flex', flexDirection: 'column', gap: 8 }}>
                      ...
                      {m.clients.length > 0 && (
                        <div style={{ display: 'flex', flexWrap: 'wrap', gap: 4 }}>
                          {m.clients.map(c => (
                            <span key={c} style={{ fontSize: 9, padding: '2px 7px', borderRadius: 5, background: '#F3F4F6', color: "#5D0565", fontWeight: 600 }}>
                              {c}
                            </span>
                          ))}
                        </div>
                      ...
                      <div style={{ display: 'flex', gap: 12, marginTop: 4 }}>
                        ...
                        <div>
                          ...
                          <p style={{ fontSize: 13, fontWeight: 900, color: "#5D0565", margin: 0 }}>{fmtRupee(m.monthlySalary)}</p>
                        </div>
                      </div>
                    </div>
                  )}
            ...
            <tbody>
              {allMembers.map((m, i) => (
                <tr key={m.employeeId} style={{ background: i % 2 === 0 ? '#fff' : '#FAFAFA' }}>
                  ...
                  <td style={{ padding: '10px 16px', fontWeight: 700, color: "#5D0565" }}>
