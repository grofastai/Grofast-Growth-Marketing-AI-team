      ...
      {[1, 2, 3, 4, 5].map(n => (
        <button key={n} type="button" onClick={() => onChange(n === value ? 0 : n)}
          ...
          <Star size={22} fill={n <= value ? "#f59e0b" : "none"} strokeWidth={1.5}
            style={{ color: n <= value ? "#f59e0b" : "#7F011F" }} />
        </button>
      ))}
      <span className="text-[12px] text-[#7F011F] font-semibold ml-1">{value ? `${value}/5` : "No rating"}</span>
    </div>
  )
}
    ...
    <div className="flex items-center gap-0.5">
      {[1, 2, 3, 4, 5].map(n => (
        <Star key={n} size={size} fill={n <= rating ? "#f59e0b" : "none"} strokeWidth={1.5}
          style={{ color: n <= rating ? "#f59e0b" : "#7F011F" }} />
      ))}
    </div>
  )
}
...
function ChipSelect({ options, selected, onChange, label }: {
  ...
  return (
    <div>
      <label className="block text-[11px] font-semibold uppercase tracking-wider text-[#7F011F] font-semibold mb-2">{label}</label>
          ...
          return (
            <button key={opt} type="button"
              ...
              style={on
                ...
                : { background: "#f9fafb", color: "#7F011F", border: "1.5px solid #e5e7eb" }
              }>
              {opt}
            </button>
          )
...
function Field({ label, children, hint }: { label: string; children: React.ReactNode; hint?: string }) {
  return (
    <div>
      <label className="block text-[11px] font-semibold uppercase tracking-wider text-[#7F011F] font-semibold mb-1.5">{label}</label>
      {children}
      {hint && <p className="text-[11px] text-[#7F011F] font-semibold mt-1">{hint}</p>}
    </div>
  )
}
    ...
    <div className="fixed inset-0 z-50 flex">
      ...
      <div className="relative ml-auto h-full w-full max-w-[480px] bg-white shadow-2xl flex flex-col overflow-hidden">
        ...
        <div className="flex items-center justify-between px-6 py-4 border-b border-gray-100">
          ...
          <button onClick={close} className="w-8 h-8 rounded-full flex items-center justify-center text-[#7F011F] font-semibold hover:text-gray-600 hover:bg-gray-100 transition-all">
            <X size={16} />
          </button>
        </div>
        ...
        <div className="flex-1 overflow-y-auto">
          {step === "type" && !editing ? (
            <div className="p-6">
              <p className="text-[13px] text-[#7F011F] font-semibold mb-5">Select the freelancer type to continue</p>
            ...
            <form id="freelancer-form" onSubmit={handleSubmit} className="p-6 flex flex-col gap-5">
              {!editing && (
                <button type="button" onClick={() => setStep("type")}
                  className="flex items-center gap-1.5 text-[12px] font-semibold text-[#7F011F] font-semibold hover:text-gray-600 transition-colors -mt-1">
                  ← Change type
                </button>
              )}
              ...
              {form.type && (
                <div>
                  <label className="block text-[11px] font-semibold uppercase tracking-wider text-[#7F011F] font-semibold mb-2">Gender</label>
                    ...
                    {(["male", "female"] as const).map(g => (
                      <button key={g} type="button"
                        ...
                        style={form.gender === g
                          ...
                          : { background: "#F9FAFB", color: "#7F011F", border: "2px solid #E5E7EB" }}>
                        {g === "male" ? "Male" : "Female"}
                      </button>
                    ))}
                  </div>
              ...
              {teamMembers.length > 0 && (
                <div>
                  <label className="block text-[11px] font-semibold uppercase tracking-wider text-[#7F011F] font-semibold mb-2">
                      ...
                      return (
                        <button key={m.id} type="button"
                          ...
                          <div className="w-4 h-4 rounded-full flex items-center justify-center text-[8px] font-black flex-shrink-0"
                            style={{ background: selected ? "rgba(255,255,255,0.3)" : "#E5E7EB", color: selected ? "#FFF" : "#7F011F" }}>
                  ...
                  {form.assignedMemberIds.length > 0 && (
                    <p className="text-[11px] text-[#7F011F] font-semibold mt-1.5">
    ...
    <div ref={ref} className="relative">
      ...
      {!isManual && (
        <button type="button" onClick={() => setOpen(o => !o)}
          ...
          style={{ color: value ? "#111827" : "#7F011F" }}>
          <span>{value || "Select client…"}</span>
          <ChevronDown size={13} className="flex-shrink-0 text-[#7F011F] font-semibold" />
        </button>
      )}
      ...
      {isManual && (
        <div className="flex gap-1.5">
          ...
          <button type="button" onClick={() => { onChange(""); setView("main"); setOpen(true) }}
            className="px-2 rounded-xl border border-gray-200 text-[#7F011F] font-semibold hover:text-gray-600 text-[11px]">✕</button>
        </div>
      )}
        ...
        <div className="absolute top-full left-0 right-0 mt-1 bg-white border border-gray-200 rounded-xl shadow-xl z-[999] overflow-hidden"
          ...
          {view === "past" && <>
            <button type="button" onClick={() => setView("main")}
              className={rowCls + " text-[#7F011F] font-semibold hover:bg-gray-50 text-[12px]"}
    ...
    <div className="fixed inset-0 z-50 flex">
      ...
      <div className="relative ml-auto h-full w-full max-w-[700px] bg-white shadow-2xl flex flex-col overflow-hidden">
        ...
        <div className="px-6 py-4 border-b border-gray-100 flex-shrink-0">
          <div className="flex items-start justify-between gap-4">
            <div>
              ...
              <div className="flex items-center gap-4 mt-1 text-[12px] text-[#7F011F] font-semibold">
            ...
            <div className="flex items-center gap-2">
              ...
              <button onClick={onClose} className="w-8 h-8 rounded-full flex items-center justify-center text-[#7F011F] font-semibold hover:text-gray-600 hover:bg-gray-100 transition-all">
                <X size={16} />
              </button>
            </div>
          </div>
        ...
        {showAdd && (
          <div className="border-b border-gray-100 flex-shrink-0 bg-gray-50">
            <form onSubmit={handleAdd} className="p-5 flex flex-col gap-4">
              <p className="text-[11px] font-bold uppercase tracking-widest text-[#7F011F] font-semibold">New Work Entry</p>
                  ...
                  <div className="flex items-center gap-6">
                    ...
                    {freelancer.cost_per_video && (
                      <div className="text-[12px]">
                        <span className="text-[#7F011F] font-semibold">Amount: </span>
              ...
              <div className="flex items-center gap-3">
                <span className="text-[11px] font-bold text-[#7F011F] font-semibold uppercase tracking-wide">Payment</span>
                <div className="flex items-center gap-2">
                  <button type="button" onClick={() => setF("payment_status", "unpaid")}
                    className="px-3 py-1.5 rounded-xl text-[12px] font-bold transition-all"
                    style={form.payment_status === "unpaid" ? { background: "#fee2e2", color: "#991b1b" } : { background: "#f3f4f6", color: "#7F011F" }}>
                    Unpaid
                  </button>
                  <button type="button" onClick={() => setF("payment_status", "paid")}
                    className="px-3 py-1.5 rounded-xl text-[12px] font-bold transition-all"
                    style={form.payment_status === "paid" ? { background: "#d1fae5", color: "#065f46" } : { background: "#f3f4f6", color: "#7F011F" }}>
                    Paid
                  </button>
                </div>
              </div>
        ...
        <div className="flex-1 overflow-y-auto p-5">
          {myEntries.length === 0 ? (
            <div className="flex flex-col items-center justify-center h-40 text-[#7F011F] font-semibold">
    ...
    <div className="bg-white border border-gray-100 rounded-2xl overflow-hidden hover:border-gray-200 transition-all">
      ...
      <div className="p-4">
        <div className="flex items-start gap-3">
          <div className="w-8 h-8 rounded-xl flex items-center justify-center flex-shrink-0 text-[12px] font-bold text-[#7F011F] font-semibold bg-gray-100">
            {idx + 1}
          </div>
          <div className="flex-1 min-w-0">
            <div className="flex items-start justify-between gap-2 flex-wrap">
              <div>
                <div className="flex items-center gap-2 flex-wrap">
                  ...
                  {entry.client_name && <span className="text-[11px] text-[#7F011F] font-semibold">· {entry.client_name}</span>}
                </div>
                <p className="text-[11px] text-[#7F011F] font-semibold mt-0.5">{new Date(entry.date).toLocaleDateString("en-IN", { day: "numeric", month: "short", year: "numeric" })}</p>
              </div>
            ...
            <div className="flex items-center gap-4 mt-2 text-[11px] text-[#7F011F] font-semibold flex-wrap">
            ...
            <div className="flex items-center gap-2 mt-3 flex-wrap">
              ...
              <button disabled={loading} onClick={async () => { if (confirm("Delete this entry?")) { setLoading(true); await onDelete(); setLoading(false) } }}
                className="flex items-center gap-1 text-[11px] font-medium px-2 py-1 rounded-lg text-[#7F011F] font-semibold hover:text-red-500 hover:bg-red-50 transition-all">
    ...
    <div className="bg-white border border-gray-100 rounded-2xl p-5 hover:border-gray-200 hover:shadow-md transition-all flex flex-col gap-4">
      ...
      <div className="flex items-start gap-3">
        ...
        <div className="flex-1 min-w-0">
          <div className="flex items-start justify-between gap-2">
            ...
            <div className="flex items-center gap-1">
              {f.status === "inactive" && (
                <span className="text-[9px] font-bold px-1.5 py-0.5 rounded-full bg-gray-100 text-[#7F011F] font-semibold">Inactive</span>
              )}
              <button onClick={onEdit} className="w-7 h-7 rounded-lg flex items-center justify-center text-[#7F011F] font-semibold hover:text-gray-700 hover:bg-gray-100 transition-all">
                <Edit2 size={12} />
              </button>
              <button onClick={() => { if (confirm(`Delete ${f.name}?`)) onDelete() }}
                className="w-7 h-7 rounded-lg flex items-center justify-center text-[#7F011F] font-semibold hover:text-red-500 hover:bg-red-50 transition-all">
                <Trash2 size={12} />
              </button>
            </div>
          </div>
      ...
      {assignedMembers.length > 0 && (
        <div className="flex flex-wrap items-center gap-1.5 pt-2 border-t border-gray-50">
          <Users2 size={11} style={{ color: "#7F011F" }} />
      ...
      <div className="grid grid-cols-3 gap-2 pt-3 border-t border-gray-50">
        <div className="text-center">
          ...
          <p className="text-[9px] font-semibold text-[#7F011F] font-semibold uppercase tracking-wide">Total Works</p>
        </div>
        <div className="text-center">
          <p className="text-[13px] font-bold text-gray-700">{totalEarned ? fmt(totalEarned) : "—"}</p>
          <p className="text-[9px] font-semibold text-[#7F011F] font-semibold uppercase tracking-wide">Earned</p>
        </div>
        <div className="text-center">
          {pending > 0
          ...
          <p className="text-[9px] font-semibold text-[#7F011F] font-semibold uppercase tracking-wide">Pending Pay</p>
        </div>
      </div>
      ...
      <div className="flex gap-2">
        ...
        <Link href={`/admin/freelancers/${f.id}`}
          className="flex-1 py-2.5 rounded-xl text-[12px] font-bold transition-all border flex items-center justify-center gap-2"
          style={{ border: "1.5px solid #E5E7EB", color: "#7F011F", background: "#F9FAFB" }}>
      ...
      <div className="bg-white border-b border-gray-100 px-6 py-5">
        <div className="max-w-7xl mx-auto flex items-center justify-between gap-4">
          <div>
            ...
            <p className="text-[13px] text-[#7F011F] font-semibold mt-0.5">{stats.total} freelancer{stats.total !== 1 ? "s" : ""} · {stats.totalWorks} work entries</p>
          </div>
      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 py-6 flex flex-col gap-6">
        ...
        <div className="grid grid-cols-2 sm:grid-cols-4 lg:grid-cols-8 gap-3">
          {STAT_CARDS.map(({ label, value, color, bg }) => (
            <div key={label} className="bg-white rounded-2xl p-4 border border-gray-100 flex flex-col gap-1">
              ...
              <p className="text-[10px] font-semibold text-[#7F011F] font-semibold uppercase tracking-wide leading-tight">{label}</p>
            </div>
          ))}
        </div>
        ...
        <div className="flex flex-col sm:flex-row items-start sm:items-center gap-3">
            ...
            {FILTER_TABS.map(tab => (
              <button key={tab.key} onClick={() => setFilter(tab.key)}
                ...
                style={filter === tab.key
                  ...
                  : { background: "#f3f4f6", color: "#7F011F" }
                }>
                {tab.label}
              </button>
            ))}
          ...
          <div className="relative flex-1 w-full sm:max-w-[260px]">
            <Search size={14} className="absolute left-3 top-1/2 -translate-y-1/2 text-[#7F011F] font-semibold" />
            <input
        ...
        {filtered.length === 0 ? (
          <div className="flex flex-col items-center justify-center bg-white rounded-2xl border border-gray-100 py-20">
            <UserCog size={40} strokeWidth={1.5} className="text-[#7F011F] font-semibold mb-3" />
            <p className="text-[15px] font-semibold text-[#7F011F] font-semibold">
              {search ? `No freelancers match "${search}"` : "No freelancers yet"}
            </p>
            <p className="text-[13px] text-[#7F011F] font-semibold mt-1">Click "Add Freelancer" to get started</p>
          </div>
        ) : (
