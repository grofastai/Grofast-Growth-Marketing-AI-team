# Notes Module V2 Implementation Plan

> **For agentic workers:** REQUIRED SUB-SKILL: Use superpowers:subagent-driven-development (recommended) or superpowers:executing-plans to implement this plan task-by-task. Steps use checkbox (`- [ ]`) syntax for tracking.

**Goal:** Enhance the existing notes module with checklist mode, labels, archive, masonry grid UI, and convert-to-task.

**Architecture:** DB migration adds 4 columns to `notes`. Server actions get new types and two new functions. The notes page server component gains archive query-param support. The client component is fully rewritten — masonry CSS columns, inline quick-create bar, checklist editor, label filter pills, archive toggle, convert-to-task popup.

**Tech Stack:** Next.js 15 App Router, Supabase (PostgreSQL + service role), TypeScript strict, Tailwind CSS v4, React `useTransition`.
...
| File | Action | Purpose |
|------|--------|---------|
| `supabase/migrations/061_notes_v2.sql` | Create | Add `note_type`, `items`, `labels`, `archived` columns + indexes |
| `lib/actions/notes.ts` | Modify | New `NoteInput`/`NoteRow` types; add `archiveNote`, `convertNoteToTask`; update `createNote`, `updateNote` |
| `app/member/notes/page.tsx` | Modify | Filter by `archived` from query param; pass notes to client |
| `app/member/notes/notes-client.tsx` | Rewrite | Masonry grid, quick-create, checklist mode, labels, archive view, convert-to-task |
...
**Files:**
- Create: `supabase/migrations/061_notes_v2.sql`

- [ ] **Step 1: Create migration file**

```sql
-- supabase/migrations/061_notes_v2.sql
ALTER TABLE notes
...
WHERE table_name = 'notes'
  AND column_name IN ('note_type','items','labels','archived');
```
...
```bash
git add supabase/migrations/061_notes_v2.sql
git commit -m "feat: add note_type, items, labels, archived to notes table"
```
...
- [ ] **Step 1: Update `NoteRow` and `NoteInput` types**

Replace the existing type definitions at the top of `lib/actions/notes.ts`:
...
- [ ] **Step 2: Update `getNotes` to accept `archived` param**

Replace the existing `getNotes` function:
...
- [ ] **Step 3: Update `createNote` to pass new fields**

Replace the `admin.from('notes').insert({...})` block inside `createNote`:
...
- [ ] **Step 4: Update `updateNote` to pass new fields**

Replace the `admin.from('notes').update({...})` block inside `updateNote`:
...
- [ ] **Step 5: Add `archiveNote` function**

Append after the existing `togglePin` function:
...
- [ ] **Step 6: Add `convertNoteToTask` function**

Append after `archiveNote`:
...
git commit -m "feat: update notes actions — checklist/labels/archive types, archiveNote, convertNoteToTask"
```
...
- [ ] **Step 1: Update page to support archived query param**

Replace the entire content of `app/member/notes/page.tsx`:
...
```bash
git add app/member/notes/page.tsx
git commit -m "feat: notes page supports ?archived=1 query param"
```

---

## Task 4: Notes Client — Full Rewrite

**Files:**
- Rewrite: `app/member/notes/notes-client.tsx`

This is the largest task. Write the complete file in one step.

- [ ] **Step 1: Write the complete new client component**

Replace the entire `app/member/notes/notes-client.tsx` with:
...
import {
  Pin, PinOff, Trash2, Plus, X, Bell, Archive, ArchiveRestore,
  CheckSquare, AlignLeft, Search, Tag, ClipboardList, Check,
} from 'lucide-react'
import {
...
function buildContent(noteType: 'text' | 'checklist', textContent: string, items: ChecklistItem[]): string {
...
function InlineEditor({
  ...
  const suggestions = allLabels.filter(l => l.toLowerCase().includes(labelInput.toLowerCase()) && !draft.labels.includes(l))
          ...
          {sortedItems.map((item, idx) => (
            <div key={item.id} style={{ display: 'flex', alignItems: 'center', gap: 8 }}>
              ...
              <input
                ...
                onKeyDown={e => { if (e.key === 'Enter') { e.preventDefault(); if (idx === sortedItems.length - 1) addItem() } }}
...
export default function NotesClient({
  ...
  // All unique labels from current notes (for autocomplete + filter)
  ...
  function handleSave() {
    ...
    if (!content.trim() && draft.note_type === 'text') { setError('Note cannot be empty'); return }
    if (draft.note_type === 'checklist' && draft.items.every(i => !i.text.trim())) { setError('Add at least one checklist item'); return }
    setError(null)
    start(async () => {
      const input: NoteInput = { ...draft, content: buildContent(draft.note_type, draft.content, draft.items) }
      if (editingNote) {
        const res = await updateNote(editingNote.id, input)
        if (!res.success) { setError(res.error ?? 'Failed to save'); return }
        setNotes(prev => prev.map(n => n.id === editingNote.id ? { ...n, ...input, title: input.title || null, updated_at: new Date().toISOString() } : n))
      } else {
    ...
    <div className="p-4 md:p-6 xl:p-8" style={{ background: '#F5F6FA', minHeight: '100vh' }}>
      ...
      {!showArchived && (
        editorOpen && !editingNote ? (
          ...
          <div style={{ background: '#FFFFFF', borderRadius: 12, border: '1.5px solid #E2E8F0', padding: '12px 16px', marginBottom: 20, display: 'flex', alignItems: 'center', gap: 10, boxShadow: '0 1px 6px rgba(0,0,0,0.06)' }}>
            ...
            <button onClick={() => openCreate('text')} title="New note"
      ...
      {filtered.length === 0 ? (
        ...
        <>
          ...
          {others.length > 0 && (
            <div>
              {pinned.length > 0 && <p style={{ fontSize: 10, fontWeight: 700, color: '#9CA3AF', textTransform: 'uppercase', letterSpacing: '0.08em', marginBottom: 8 }}>Other Notes</p>}
              <div style={{ columnCount: 3, columnGap: 12 }} className="notes-grid">
                {others.map(note => (
                  <NoteCard key={note.id} note={note} onEdit={openEdit} onPin={handlePin} onArchive={handleArchive} onDelete={handleDelete} showArchived={showArchived} />
                ))}
...
- [ ] **Step 2: Fix syntax error in InlineEditor**

Find the line with `'marginLeft: 'auto', display: 'flex', gap: 8' }}>` (the stray single-quote before the closing brace) and fix it:
...
```bash
git add app/member/notes/notes-client.tsx
git commit -m "feat: notes v2 — masonry grid, checklist, labels, archive, convert to task"
```
...
- [x] **Spec coverage:** DB migration ✓ · `archiveNote` ✓ · `convertNoteToTask` ✓ · masonry grid ✓ · quick-create bar ✓ · checklist mode ✓ · label filter ✓ · archive view ✓ · convert-to-task popup ✓ · pinned section ✓ · search ✓
- [x] **No placeholders:** All code is complete
- [x] **Type consistency:** `ChecklistItem` defined in Task 2 Step 1, used in Tasks 2 and 4. `NoteRow.items: ChecklistItem[]` consistent throughout. `archiveNote(id, archived)` signature matches usage in card component. `convertNoteToTask(noteId, taskTitle, dueDate?)` matches `ConvertPopup` call.
- [x] **Syntax error noted:** The stray quote in `gap: 8' }}` is flagged in Task 4 Step 2 for explicit fixing.
