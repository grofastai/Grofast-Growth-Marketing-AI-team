...
function TaskCard({ task, onMove, onDelete, isMoving, isDeleting }: {
  ...
  return (
    ...
    >
      ...
      {task.description && (
        <p className="text-[11px] mb-2.5 leading-relaxed" style={{ color: "#000000" }}>{task.description}</p>
      )}
      <div className="flex flex-wrap gap-1.5 mb-2.5">
        ...
        {task.due_date && (
          <span className="flex items-center gap-0.5 text-[10px] px-2 py-0.5 rounded-full"
            style={{ background: overdue ? "rgba(222,26,26,0.08)" : "#F3F4F6", color: overdue ? "#de1a1a" : "#000000" }}>
        ...
        {task.users && (
          task.acknowledged_at ? (
            ...
            <span className="text-[10px] px-2 py-0.5 rounded-full font-semibold"
              style={{ background: "rgba(156,163,175,0.12)", color: "#000000" }}>Not seen</span>
          )
        )}
      </div>
      ...
      {(task.attachments ?? []).length > 0 && (
        <div style={{ marginBottom: 8, display: "flex", flexDirection: "column", gap: 4 }}>
          <div style={{ display: "flex", alignItems: "center", gap: 4, marginBottom: 2 }}>
            <Paperclip size={9} style={{ color: "#000000" }} />
            <span style={{ fontSize: 9, fontWeight: 700, color: "#000000", textTransform: "uppercase", letterSpacing: "0.06em" }}>
          ...
          {(task.attachments ?? []).map((att, i) => {
            ...
            return (
              <a key={i} href={att.url} target="_blank" rel="noopener noreferrer"
                ...
                <ExternalLink size={9} style={{ color: "#000000", flexShrink: 0 }} />
              </a>
            )
          })}
        </div>
      ...
      <div className="flex gap-1.5 opacity-0 group-hover:opacity-100 transition-opacity pt-2"
        ...
        {prev && (
          <button onClick={() => onMove(task.id, prev)} disabled={isMoving}
            ...
            style={{ background: "#F9FAFB", color: "#000000", border: "1px solid #E5E7EB" }}>← Back</button>
        )}
        {next && (
        ...
        {STAT_CARDS.map(card => (
          <div key={card.label} style={{
            ...
            <div style={{ display: "flex", alignItems: "flex-start", justifyContent: "space-between", position: "relative" }}>
              <div>
                <p style={{ fontSize: 12, color: "#000000", fontWeight: 600, margin: "0 0 10px", textTransform: "uppercase", letterSpacing: "0.08em" }}>{card.label}</p>
                ...
                <div key={col.id} style={{
                  ...
                  <div style={{ padding: "16px 18px 14px", borderBottom: "1px solid #F5F5F5", display: "flex", alignItems: "center", gap: 12 }}>
                    <div style={{
                      ...
                      color: isUnassigned ? "#000000" : "#FFFFFF",
                    ...
                    <div style={{ flex: 1, minWidth: 0 }}>
                      ...
                      {col.team && <p style={{ fontSize: 10, color: "#000000", margin: "2px 0 0", overflow: "hidden", textOverflow: "ellipsis", whiteSpace: "nowrap" }}>{col.team}</p>}
                    </div>
                    <div style={{
                      minWidth: 28, height: 28, borderRadius: 8, fontSize: 12, fontWeight: 800,
                      display: "flex", alignItems: "center", justifyContent: "center",
                      background: isUnassigned ? "#F3F4F6" : `${tc.from}18`,
                      color: isUnassigned ? "#000000" : tc.from,
                      padding: "0 6px",
                  ...
                  <div style={{ flex: 1, display: "flex", flexDirection: "column" }}>
                    {col.tasks.length === 0 ? (
                      <div style={{ flex: 1, display: "flex", flexDirection: "column", alignItems: "center", justifyContent: "center", gap: 8, padding: "24px 16px" }}>
                        ...
                        <p style={{ fontSize: 12, color: "#000000", fontWeight: 600, margin: 0 }}>No tasks assigned yet</p>
                      </div>
                    ) : (
          ...
          <div className="md:hidden">
            ...
            <div className="flex flex-col gap-3">
              {tasks.filter(t => t.status === mobileCol).length === 0 ? (
                <div style={{ textAlign: "center", padding: "40px 0", color: "#000000", fontSize: 13, fontWeight: 600 }}>
                  No tasks here
                </div>
              ) : (
              ...
              <div key={col.key} style={{
                ...
                {col.tasks.length === 0 ? (
                  <div style={{ flex: 1, display: "flex", alignItems: "center", justifyContent: "center", flexDirection: "column", gap: 8 }}>
                    ...
                    <p style={{ fontSize: 12, color: "#000000", fontWeight: 600 }}>No tasks here</p>
                  </div>
                ) : (
                  <div style={{ display: "flex", flexDirection: "column", gap: 10, padding: "14px 14px" }}>
                    {col.tasks.map(task => (
        ...
        <button
          ...
          <div style={{ display: "flex", alignItems: "center", gap: 12 }}>
            ...
            <div style={{ textAlign: "left" }}>
              ...
              <p style={{ fontSize: 11, color: "#000000", margin: 0 }}>{archivedTasks.length} completed task{archivedTasks.length !== 1 ? "s" : ""}</p>
            </div>
          </div>
          <div style={{ display: "flex", alignItems: "center", gap: 8 }}>
            ...
            {archiveOpen ? <ChevronUp size={16} style={{ color: "#000000" }} /> : <ChevronDown size={16} style={{ color: "#000000" }} />}
          </div>
        </button>
        ...
        {archiveOpen && (
          <div style={{ background: "#FAFBFC", border: "1px solid #EBEDF2", borderTop: "none", borderRadius: "0 0 20px 20px", padding: "16px 18px" }}>
            {archivedTasks.length === 0 ? (
              <div style={{ textAlign: "center", padding: "32px 0", color: "#000000", fontSize: 13, fontWeight: 600 }}>
        ...
        <div style={{ position: "fixed", inset: 0, zIndex: 50, display: "flex", alignItems: "center", justifyContent: "center", background: "rgba(0,0,0,0.5)", backdropFilter: "blur(8px)" }}>
          <div style={{ width: "100%", maxWidth: 520, maxHeight: "90vh", borderRadius: 24, margin: "0 16px", background: "#FFFFFF", boxShadow: "0 24px 80px rgba(0,0,0,0.22)", display: "flex", flexDirection: "column", overflow: "hidden" }}>
            <div style={{ display: "flex", alignItems: "center", justifyContent: "space-between", padding: "24px 28px 20px", flexShrink: 0 }}>
              <div style={{ display: "flex", alignItems: "center", gap: 12 }}>
                ...
                <div>
                  ...
                  <p style={{ fontSize: 11, color: "#000000", margin: 0 }}>Assign work to your team</p>
                </div>
              </div>
            ...
            <form action={action} style={{ display: "flex", flexDirection: "column", flex: 1, minHeight: 0 }}>
              ...
              <div>
                <label style={{ display: "block", fontSize: 10, fontWeight: 700, color: "#000000", textTransform: "uppercase", letterSpacing: "0.14em", marginBottom: 6 }}>Title *</label>
                <input name="title" required placeholder="Task title…" className="ti" />
              </div>
              <div>
                <label style={{ display: "block", fontSize: 10, fontWeight: 700, color: "#000000", textTransform: "uppercase", letterSpacing: "0.14em", marginBottom: 6 }}>Description</label>
                <textarea name="description" rows={2} placeholder="Optional details…" className="ti" style={{ resize: "none" }} />
              </div>
              <div>
                <label style={{ display: "block", fontSize: 10, fontWeight: 700, color: "#000000", textTransform: "uppercase", letterSpacing: "0.14em", marginBottom: 8 }}>
                ...
                {members.length === 0 ? (
                  <p style={{ fontSize: 12, color: "#000000" }}>No team members yet</p>
                ) : (
                      ...
                      return (
                        <button key={m.id} type="button" onClick={() => toggleMember(m.id)}
                          style={{
                            ...
                            color: sel ? "#de1a1a" : "#000000",
                          }}>
                          <span style={{ width: 22, height: 22, borderRadius: "50%", fontSize: 9, fontWeight: 800, display: "flex", alignItems: "center", justifyContent: "center", background: sel ? "rgba(222,26,26,0.15)" : "#E5E7EB", color: sel ? "#de1a1a" : "#000000" }}>
              ...
              <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: 12 }}>
                <div>
                  <label style={{ display: "block", fontSize: 10, fontWeight: 700, color: "#000000", textTransform: "uppercase", letterSpacing: "0.14em", marginBottom: 6 }}>Priority</label>
                ...
                <div>
                  <label style={{ display: "block", fontSize: 10, fontWeight: 700, color: "#000000", textTransform: "uppercase", letterSpacing: "0.14em", marginBottom: 6 }}>Project</label>
              ...
              <div>
                <label style={{ display: "block", fontSize: 10, fontWeight: 700, color: "#000000", textTransform: "uppercase", letterSpacing: "0.14em", marginBottom: 6 }}>Due Date</label>
                <input name="due_date" type="date" max="2099-12-31" className="ti" style={{ colorScheme: "light" }} />
              </div>

              {/* Manager Note */}
              <div>
                <label style={{ display: "block", fontSize: 10, fontWeight: 700, color: "#D97706", textTransform: "uppercase", letterSpacing: "0.14em", marginBottom: 6 }}>📌 Manager Note <span style={{ color: "#000000", fontWeight: 600, textTransform: "none", letterSpacing: 0 }}>(shown on task card)</span></label>
                <textarea
                  name="manager_note"
                  rows={2}
                  value={managerNote}
              ...
              {/* Checklist */}
              <div>
                <label style={{ display: "block", fontSize: 10, fontWeight: 700, color: "#000000", textTransform: "uppercase", letterSpacing: "0.14em", marginBottom: 6 }}>
                  Checklist <span style={{ color: "#000000", fontWeight: 600, textTransform: "none", letterSpacing: 0 }}>(optional — shown to assignee)</span>
                </label>
                  ...
                  <div style={{ display: "flex", flexDirection: "column", gap: 6 }}>
                    {checklistItems.map((item, idx) => (
                      <div key={idx} style={{ display: "flex", alignItems: "center", gap: 8, padding: "7px 12px", borderRadius: 10, background: "#F9FAFB", border: "1px solid #E5E7EB" }}>
                        <span style={{ fontSize: 11, color: "#000000", flex: 1 }}>☐ {item}</span>
              ...
              <div>
                <label style={{ display: "block", fontSize: 10, fontWeight: 700, color: "#000000", textTransform: "uppercase", letterSpacing: "0.14em", marginBottom: 6 }}>
                  Attachments / Links <span style={{ color: "#000000", fontWeight: 600, textTransform: "none", letterSpacing: 0 }}>(optional)</span>
                </label>
              ...
              <div style={{ display: "flex", gap: 10, padding: "16px 28px 24px", borderTop: "1px solid #F0F0F0", flexShrink: 0 }}>
                <button type="button" onClick={() => { setShowForm(false); setSelectedMembers([]); setMediaClientType(""); setMediaBrand(""); setMediaCustomClient(""); setManagerNote(""); setChecklistItems([]); setNewCheckItem(""); setAttachmentLinks([]); setNewAttachUrl("") }}
                  style={{ flex: 1, padding: "12px 0", borderRadius: 12, fontSize: 13, fontWeight: 600, border: "1.5px solid #E5E7EB", background: "#F9FAFB", color: "#000000", cursor: "pointer" }}>
                  Cancel
                </button>
