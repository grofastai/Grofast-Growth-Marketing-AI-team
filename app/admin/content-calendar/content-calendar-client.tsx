...
function DonutChart({ total, posted, inProgress, ready, pending }: {
  ...
  if (total === 0) return (
    <svg width="130" height="130">
      ...
      <text x={cx} y={cy + 13} textAnchor="middle" fontSize="9" fill="#1b365d">Total Content</text>
    </svg>
  )
  const segs = [
        ...
        <div style={{ flex: 1, minWidth: 260, display: "flex", gap: 4, background: "#FFFFFF", borderRadius: 18, padding: 4, border: "1px solid #E5E7EB", boxShadow: "0 2px 16px rgba(0,0,0,0.06)" }}>
          {([
            ...
            <button key={mode} onClick={() => setContentMode(mode)} style={{
              ...
              color: contentMode === mode ? "#FFF" : "#1b365d",
        ...
        <div style={{ background: "#FFFFFF", borderRadius: 18, padding: "14px 18px", border: "1px solid #E5E7EB", boxShadow: "0 2px 16px rgba(0,0,0,0.06)", display: "flex", flexDirection: "column", justifyContent: "center", minWidth: 200 }}>
          ...
          <div style={{ position: "relative" }}>
            <select value={memberFilter} onChange={e => setMemberFilter(e.target.value)}
              style={{ width: "100%", padding: "8px 30px 8px 12px", borderRadius: 10, border: `1.5px solid ${memberFilter === "all" ? "#E5E7EB" : "#DE1A1A"}`, background: memberFilter === "all" ? "#FAFAFA" : "rgba(222,26,26,0.04)", fontSize: 13, fontWeight: 700, color: memberFilter === "all" ? "#1b365d" : "#DE1A1A", cursor: "pointer", outline: "none", appearance: "none" }}>
              <option value="all">👥 All Team Members</option>
              {members.map(m => <option key={m.id} value={m.id}>👤 {m.name}</option>)}
            </select>
            <ChevronDown size={11} style={{ position: "absolute", right: 10, top: "50%", transform: "translateY(-50%)", color: "#1b365d", pointerEvents: "none" }} />
          </div>
        </div>
      </div>
            ...
            <div style={{ display: "flex", alignItems: "center", justifyContent: "space-between", padding: "14px 18px", borderBottom: "1px solid #F3F4F6", flexWrap: "wrap", gap: 10 }}>
              <div style={{ display: "flex", alignItems: "center", gap: 6 }}>
                <button onClick={prevMonth} style={{ width: 30, height: 30, borderRadius: 8, border: "1px solid #E5E7EB", background: "#FAFAFA", cursor: "pointer", display: "flex", alignItems: "center", justifyContent: "center" }}>
                  <ChevronLeft size={13} color="#1b365d" />
                </button>
                <button onClick={() => { setYear(initialYear); setMonth(initialMonth); setSelectedDate(today) }}
                  style={{ padding: "6px 13px", fontSize: 11, fontWeight: 700, color: "#1b365d", background: "#F3F4F6", borderRadius: 8, border: "none", cursor: "pointer" }}>
                  Today
                </button>
                <button onClick={nextMonth} style={{ width: 30, height: 30, borderRadius: 8, border: "1px solid #E5E7EB", background: "#FAFAFA", cursor: "pointer", display: "flex", alignItems: "center", justifyContent: "center" }}>
                  <ChevronRight size={13} color="#1b365d" />
                </button>
              </div>
                ...
                {clientOptions.length > 0 && (
                  <div style={{ position: "relative" }}>
                    <select value={clientFilter} onChange={e => setClientFilter(e.target.value)}
                      style={{ fontSize: 11, fontWeight: 600, color: clientFilter === "all" ? "#1b365d" : "#DE1A1A", background: clientFilter === "all" ? "#F9FAFB" : "rgba(222,26,26,0.05)", border: `1.5px solid ${clientFilter === "all" ? "#E5E7EB" : "rgba(222,26,26,0.3)"}`, borderRadius: 8, padding: "5px 22px 5px 9px", cursor: "pointer", outline: "none", appearance: "none" }}>
                      <option value="all">All Clients</option>
                      {clientOptions.map(c => <option key={c} value={c}>{c}</option>)}
                    </select>
                    <ChevronDown size={10} style={{ position: "absolute", right: 6, top: "50%", transform: "translateY(-50%)", color: "#1b365d", pointerEvents: "none" }} />
                  </div>
                )}
            ...
            {view === "calendar" ? (
              <>
                ...
                <div style={{ display: "grid", gridTemplateColumns: "repeat(7, 1fr)" }}>
                  {DAYS.map(d => (
                    <div key={d} style={{ padding: "8px 0", textAlign: "center", fontSize: 10, fontWeight: 700, color: "#1b365d", textTransform: "uppercase", letterSpacing: "0.07em", borderBottom: "1px solid #F3F4F6" }}>{d}</div>
                  ))}
                </div>
                {/* Grid cells */}
                <div style={{ display: "grid", gridTemplateColumns: "repeat(7, 1fr)" }}>
                    ...
                    return (
                      <div key={i} onClick={() => setSelectedDate(ds)} style={{
                        ...
                        <div style={{ display: "flex", alignItems: "center", justifyContent: "space-between", marginBottom: isMobile ? 3 : 4 }}>
                          <span style={{ fontSize: isMobile ? 10 : 12, fontWeight: isToday ? 900 : 500, width: isMobile ? 18 : 22, height: isMobile ? 18 : 22, display: "flex", alignItems: "center", justifyContent: "center", borderRadius: "50%", background: isToday ? "#DE1A1A" : "transparent", color: isToday ? "#FFF" : isSelected ? "#DE1A1A" : "#1b365d" }}>
                            {day}
                          </span>
                          {total > 0 && (
                        ...
                        {!isMobile && more > 0 && (
                          <span style={{ fontSize: 9, fontWeight: 700, color: "#1b365d", paddingLeft: 5 }}>+{more} more</span>
                        )}
                      </div>
                    )
                  })}
                ...
                <div style={{ padding: "10px 18px", borderTop: "1px solid #F3F4F6", display: "flex", gap: 14, flexWrap: "wrap", alignItems: "center" }}>
                  <span style={{ fontSize: 10, color: "#1b365d", fontWeight: 600 }}>Legend:</span>
              ...
              <div>
                <div style={{ padding: "14px 18px", borderBottom: "1px solid #F3F4F6", display: "flex", alignItems: "center", justifyContent: "space-between", flexWrap: "wrap", gap: 8 }}>
                  ...
                  {memberFilter !== "all" && (
                    <button onClick={() => setMemberFilter("all")} style={{ fontSize: 11, fontWeight: 700, padding: "3px 10px", borderRadius: 99, border: "1.5px solid #E5E7EB", background: "#F3F4F6", color: "#1b365d", cursor: "pointer" }}>
                      Show All ×
                    </button>
                  )}
                </div>
                {filteredPosts.length === 0 ? (
                  <div style={{ padding: "60px 24px", textAlign: "center", color: "#1b365d", fontSize: 13 }}>No content scheduled this month.</div>
                ) : (
                  <div>
                      ...
                      return (
                        <div key={p.id} style={{ display: "flex", alignItems: "center", gap: 12, padding: "12px 18px", borderTop: i > 0 ? "1px solid #F9FAFB" : "none", flexWrap: "wrap" }}>
                          ...
                          <div style={{ display: "flex", flexDirection: "column", alignItems: "flex-end", gap: 3, flexShrink: 0 }}>
                            <span style={{ fontSize: 11, color: "#1b365d", whiteSpace: "nowrap" }}>{p.scheduled_date}</span>
              ...
              {[
                ...
                <button key={a.label} onClick={a.action}
                  ...
                  <div style={{ flex: 1, minWidth: 0 }}>
                    ...
                    <p style={{ fontSize: 10, color: "#1b365d", margin: "2px 0 0", whiteSpace: "nowrap", overflow: "hidden", textOverflow: "ellipsis" }}>{a.sub}</p>
                  </div>
                </button>
              ))}
            </div>
          ...
          <div style={{ display: "flex", flexDirection: "column", gap: 14, ...(isMobile ? {} : { position: "sticky", top: 24 }) }}>
              ...
              {(() => {
                ...
                if (total === 0) return (
                  <div style={{ padding: "28px 20px", textAlign: "center" }}>
                    ...
                    <p style={{ fontSize: 13, color: "#1b365d", margin: "0 0 4px", fontWeight: 600 }}>No content scheduled</p>
                    <p style={{ fontSize: 11, color: "#1b365d", margin: 0 }}>Click Add Post to schedule something</p>
                  </div>
                )
                return (
                      ...
                      return (
                        <div key={p.id} style={{ padding: "12px 20px", borderBottom: "1px solid #F3F4F6" }}>
                          ...
                          <div style={{ display: "flex", alignItems: "center", gap: 10, marginBottom: 10 }}>
                            ...
                            <div style={{ flex: 1, minWidth: 0 }}>
                              <p style={{ fontSize: 13, fontWeight: 700, color: "#111827", margin: 0, overflow: "hidden", textOverflow: "ellipsis", whiteSpace: "nowrap" }}>{p.title}</p>
                              <p style={{ fontSize: 11, color: shoot ? "#9B6BFF" : "#1b365d", margin: "2px 0 0", whiteSpace: "nowrap", overflow: "hidden", textOverflow: "ellipsis", fontWeight: shoot ? 600 : 400 }}>
            ...
            <div style={{ background: "#FFFFFF", borderRadius: 18, border: "1px solid #E5E7EB", overflow: "hidden" }}>
              ...
              {upcomingPosts.length === 0 ? (
                <p style={{ fontSize: 12, color: "#1b365d", textAlign: "center", padding: "16px", margin: 0 }}>No upcoming posts</p>
              ) : (
                <div>
                    ...
                    return (
                      <div key={p.id} style={{ display: "flex", alignItems: "center", gap: 10, padding: "10px 18px", borderBottom: "1px solid #F9FAFB", cursor: "pointer" }} onClick={() => openEdit(p)}>
                        ...
                        <div style={{ flex: 1, minWidth: 0 }}>
                          ...
                          <p style={{ fontSize: 10, color: shoot ? "#9B6BFF" : "#1b365d", margin: "1px 0 0", fontWeight: shoot ? 600 : 400 }}>{shoot ? "📹 Shoot · " : ""}{p.scheduled_date === today ? "Today" : p.scheduled_date.slice(5)}</p>
                        </div>
          ...
          <div style={{ position: "relative", background: "#FFFFFF", borderRadius: 20, padding: 28, width: "100%", maxWidth: 580, maxHeight: "92vh", overflowY: "auto", boxShadow: "0 20px 60px rgba(0,0,0,0.2)" }}>
            <div style={{ display: "flex", alignItems: "center", justifyContent: "space-between", marginBottom: 20 }}>
              <div>
                ...
                {!isEdit && <p style={{ fontSize: 12, color: "#1b365d", margin: "2px 0 0" }}>
              ...
              <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: 14, marginBottom: 4 }}>
                {[
                  ...
                  <button key={opt.key} type="button"
                    ...
                    <span style={{ fontSize: 11, color: "#1b365d" }}>{opt.sub}</span>
                  </button>
                ))}
              </div>
            )}
            {!isEdit && schedType && (
              <button type="button" onClick={() => setSchedType("")}
                style={{ fontSize: 11, fontWeight: 700, color: "#1b365d", background: "none", border: "none", cursor: "pointer", padding: "0 0 6px", textAlign: "left" }}>
                ← Change type
              </button>
            )}
            ...
            <form onSubmit={isEdit ? handleEdit : handleCreate} style={{ display: isEdit || schedType ? "flex" : "none", flexDirection: "column", gap: 16 }}>
              ...
              <div>
                <label style={LABEL}>Date{!isEdit ? "s" : ""} * {!isEdit && <span style={{ fontWeight: 600, color: "#1b365d", textTransform: "none" }}>— select one or more</span>}</label>
              ...
              {(isEdit ? contentType !== "shoot" : schedType !== "shoot") && (
                <div>
                  <label style={LABEL}>Post Time <span style={{ fontWeight: 600, color: "#1b365d", textTransform: "none" }}>(optional)</span></label>
              ...
              {(isEdit ? contentType === "shoot" : schedType === "shoot") ? (
                <div>
                  <label style={LABEL}>Assign To <span style={{ fontWeight: 600, color: "#1b365d", textTransform: "none" }}>(select all crew members)</span></label>
                    ...
                    {members.map(m => {
                      ...
                      return (
                        <button key={m.id} type="button"
                          ...
                          style={{ padding: "6px 14px", borderRadius: 20, border: `1.5px solid ${sel ? "#DE1A1A" : "#E2E8F0"}`, background: sel ? "rgba(222,26,26,0.08)" : "#FAFAFA", color: sel ? "#DE1A1A" : "#1b365d", fontSize: 12, fontWeight: 700, cursor: "pointer" }}>
                          {sel ? "✓ " : ""}{m.name}
                        </button>
                      )
                    })}
              ...
              <div>
                <label style={LABEL}>Instructions <span style={{ fontWeight: 600, color: "#1b365d", textTransform: "none" }}>(optional)</span></label>
              ...
              <div style={{ display: "flex", gap: 10 }}>
                <button type="button" onClick={() => setModalMode(null)}
                  style={{ flex: 1, padding: "12px", borderRadius: 10, border: "1px solid #E2E8F0", background: "#FAFAFA", fontSize: 13, fontWeight: 700, cursor: "pointer", color: "#1b365d" }}>
                  Cancel
                </button>
