    ...
    <div style={{ display: "flex", gap: 0, flex: 1, minHeight: 0 }}>
      ...
      {!hideLeftPanel && (
        <div style={{ width: 220, flexShrink: 0, borderRight: "1px solid #F0F1F5", background: "#FAFAFA", padding: "16px 0", overflowY: "auto" }}>
          <p style={{ fontSize: 10, fontWeight: 700, color: "#7F011F", textTransform: "uppercase", letterSpacing: "0.08em", padding: "0 16px 8px" }}>Members</p>
          {members.map(m => {
          ...
          {members.length === 0 && <p style={{ fontSize: 12, color: "#7F011F", padding: "0 16px" }}>No members yet</p>}
        </div>
      )}
      ...
      <div style={{ flex: 1, minHeight: 0, overflow: "hidden", display: "flex", flexDirection: "column" }}>
        ...
        <div style={{ flex: 1, minHeight: 0, overflowY: "auto", padding: "14px 16px", display: "flex", flexDirection: "column", gap: 8 }}>
          {filteredEntries.length === 0 ? (
            <div style={{ display: "flex", flexDirection: "column", alignItems: "center", justifyContent: "center", height: "100%", color: "#7F011F", gap: 8 }}>
              ...
              return (
                <div key={key}
                  ...
                  <div style={{ flex: 1, minWidth: 0, display: "flex", alignItems: "center", gap: 10, overflow: "hidden" }}>
                    ...
                    {e.video_duration && <span style={{ fontSize: 11, color: "#7F011F", whiteSpace: "nowrap", flexShrink: 0 }}>· {e.video_duration}</span>}
                    {e.duration_hours ? <span style={{ fontSize: 11, color: "#7F011F", whiteSpace: "nowrap", flexShrink: 0 }}>· {fmtH(e.duration_hours)}</span> : null}
                  </div>

                  {/* Price input + save */}
                  <div style={{ display: "flex", alignItems: "center", gap: 8, flexShrink: 0 }}>
                    <div style={{ position: "relative" }}>
                      <span style={{ position: "absolute", left: 9, top: "50%", transform: "translateY(-50%)", fontSize: 12, fontWeight: 800, color: hasPendingChange ? "#DC143C" : "#7F011F", pointerEvents: "none" }}>₹</span>
                      <input
                        type="number"
                        min="0"
                        placeholder="0"
