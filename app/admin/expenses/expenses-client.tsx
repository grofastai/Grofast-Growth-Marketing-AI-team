...
function ClientExpenseModalBody({ clients, selectedMonth, editing, onClose }: {
  ...
  return (
      <div className="space-y-4">
        <div>
          <label className="text-[11px] font-bold uppercase tracking-wider" style={{ color: "#1b365d" }}>Client</label>
          ...
          {(["ad", "software", "other"] as const).map(t => (
            <button key={t} onClick={() => setType(t)}
              ...
              style={{
                ...
                color: type === t ? TYPE_COLOR[t] : "#1b365d",
              }}>
              {TYPE_ICON[t]}
              {t}
            </button>
        ...
        <div className="grid grid-cols-2 gap-3">
          <div>
            <label className="text-[11px] font-bold uppercase tracking-wider" style={{ color: "#1b365d" }}>Date</label>
            <input type="date" max="2099-12-31" value={date} onChange={e => setDate(e.target.value)}
              className="mt-1 w-full rounded-xl px-3 py-2.5 text-[13px] outline-none"
              style={{ background: "#F9FAFB", border: "1px solid #E5E7EB", color: "#111111" }} />
          </div>
          <div>
            <label className="text-[11px] font-bold uppercase tracking-wider" style={{ color: "#1b365d" }}>Amount (₹)</label>
        ...
        <div>
          <label className="text-[11px] font-bold uppercase tracking-wider" style={{ color: "#1b365d" }}>Notes (optional)</label>
...
function CommonExpenseModalBody({ selectedMonth, overheadDivisor, editing, onClose }: {
  ...
  return (
      <div className="space-y-4">
        <div>
          <label className="text-[11px] font-bold uppercase tracking-wider" style={{ color: "#1b365d" }}>Expense Name</label>
        ...
        {previewShare > 0 && (
          <div className="flex items-center gap-2 px-3 py-2.5 rounded-xl"
            ...
            <span className="text-[12px]" style={{ color: "#1b365d" }}>
              ...
              <span style={{ color: "#1b365d" }}> ÷ {overheadDivisor}</span>
            </span>
          </div>
        )}
        <div>
    ...
    <FlatCard className="overflow-hidden">
      ...
      {shoots.length === 0 ? (
        <div className="flex flex-col items-center py-16">
          ...
          <p className="text-[13px]" style={{ color: "#1b365d" }}>No shoots logged this month</p>
        </div>
      ) : (
          ...
          {(() => {
            ...
            return (
              <div
                ...
                <p className="text-[11px] font-bold uppercase tracking-wider" style={{ color: isTravelActive ? "rgba(255,255,255,0.75)" : "#1b365d", position: "relative" }}>Total Expenses</p>
                ...
                <p className="text-[11px] mt-3" style={{ color: isTravelActive ? "rgba(255,255,255,0.7)" : "#1b365d", position: "relative" }}>{MONTHS_SHORT[mo - 1]} {yr}</p>
              </div>
            )
          })()}
          {([
            ...
            return (
              <FlatCard key={s.label} className="p-5 transition-all" onClick={() => setActiveTab(s.key)}
                ...
                <div className="flex items-center gap-2 mb-2" style={{ position: "relative" }}>
                  ...
                  <p className="text-[12px] font-bold" style={{ color: isActive ? "rgba(255,255,255,0.85)" : "#1b365d" }}>{s.label}</p>
                </div>
                ...
                <p className="text-[10px] mt-1" style={{ color: isActive ? "rgba(255,255,255,0.7)" : "#1b365d", position: "relative" }}>{pctOfTotal(s.value)}% of total</p>
        ...
        <div className="flex flex-col md:flex-row items-stretch gap-3">
          <FlatCard className="p-5 flex-1 flex items-center gap-4">
            ...
            <div>
              ...
              <p className="text-[12px] mt-1" style={{ color: "#1b365d" }}>Monitor all your client wise expenses in one place. Analyse, compare and optimise your business spending.</p>
            </div>
          </FlatCard>
            ...
            {([
              ...
              <button key={t.key} onClick={() => setActiveTab(t.key)}
                ...
                style={{
                  ...
                  color: activeTab === t.key ? "#fff" : "#1b365d",
          ...
          {activeTab === "summary" && (
            <div className="flex items-center gap-2">
              <select value={clientFilter} onChange={e => setClientFilter(e.target.value)}
                ...
                style={{ background: "#fff", border: "1px solid #EDEDED", color: "#1b365d" }}>
                <option value="all">All Clients</option>
                {clientSummaryRows.map(r => <option key={r.name} value={r.name}>{r.name}</option>)}
              </select>
              <div className="relative">
                <Search size={13} style={{ position: "absolute", left: 10, top: "50%", transform: "translateY(-50%)", color: "#1b365d" }} />
          ...
          <FlatCard className="overflow-hidden">
            ...
            {clientExpenses.length === 0 ? (
              <div className="flex flex-col items-center py-12">
                ...
                <p className="text-[13px]" style={{ color: "#1b365d" }}>No client expenses this month</p>
              </div>
            ) : (
              <div className="overflow-x-auto">
                <table style={{ minWidth: 720 }} className="w-full">
                        ...
                        <td className="px-6 py-3">
                          <div className="flex items-center justify-end gap-2">
                            <button onClick={() => { setEditClient(e); setModal("client") }} className="opacity-40 hover:opacity-90 transition-opacity">
                              <Pencil size={13} style={{ color: "#1b365d" }} />
                            </button>
                  ...
                  <tfoot>
                    <tr style={{ background: "#FAFAFA", borderTop: "2px solid #EDEDED" }}>
                      <td colSpan={4} className="px-6 py-3 text-[11px] font-black uppercase tracking-wider" style={{ color: "#1b365d" }}>Total</td>
        ...
        {activeTab === "common" && (
          <FlatCard className="overflow-hidden">
            ...
            {commonExpenses.length === 0 ? (
              <div className="flex flex-col items-center py-12">
                <AlertCircle size={28} style={{ color: "#E5E7EB" }} className="mb-2" />
                <p className="text-[13px]" style={{ color: "#1b365d" }}>No common expenses this month</p>
              </div>
            ) : (
              <div className="overflow-x-auto">
                <table style={{ minWidth: 640 }} className="w-full">
                  ...
                  <tfoot>
                    <tr style={{ background: "#FAFAFA", borderTop: "2px solid #EDEDED" }}>
                      <td colSpan={3} className="px-6 py-3 text-[11px] font-black uppercase tracking-wider" style={{ color: "#1b365d" }}>Total</td>
          ...
          <FlatCard className="overflow-hidden">
            ...
            <div className="overflow-x-auto">
              <table style={{ minWidth: 640 }} className="w-full">
                <thead>
                  <tr style={{ background: "#FAFAFA", borderBottom: "1px solid #EDEDED" }}>
                    <th className="px-6 py-3 text-left text-[10px] font-bold uppercase tracking-wider whitespace-nowrap" style={{ color: "#1b365d" }}>Client / Brand</th>
                    ...
                    <th className="px-6 py-3 text-right text-[10px] font-bold uppercase tracking-wider whitespace-nowrap" style={{ color: "#1b365d" }}>Action</th>
                  </tr>
                </thead>
                <tbody>
                  {filteredSummaryRows.length === 0 ? (
                    <tr><td colSpan={6} className="px-6 py-10 text-center text-[13px]" style={{ color: "#1b365d" }}>No clients match this filter</td></tr>
                    ...
                    return (
                      <tr key={row.name} style={{ borderBottom: i < filteredSummaryRows.length - 1 ? "1px solid #F5F5F5" : "none" }}>
                        ...
                        <td className="px-4 py-3 text-right">
                          <div className="text-[13px] font-black" style={{ color: row.empCost > 0 ? "#059669" : "#1b365d", fontVariantNumeric: "tabular-nums" }}>{fmtRupee(row.empCost)}</div>
                        </td>
                        <td className="px-4 py-3 text-right">
                          <div className="text-[13px] font-black" style={{ color: row.direct > 0 ? "#6366F1" : "#1b365d", fontVariantNumeric: "tabular-nums" }}>{fmtRupee(row.direct)}</div>
                        </td>
                        <td className="px-4 py-3 text-right">
                        ...
                        <td className="px-6 py-3">
                          <div className="flex items-center justify-end gap-2">
                            ...
                            <button className="w-7 h-7 rounded-lg flex items-center justify-center flex-shrink-0" style={{ color: "#1b365d" }}>
                ...
                <tfoot>
                  <tr style={{ background: "#FAFAFA", borderTop: "2px solid #EDEDED" }}>
                    <td className="px-6 py-3 text-[11px] font-black uppercase tracking-wider" style={{ color: "#1b365d" }}>Total</td>
          ...
          <div className="space-y-5">
            <div className="grid grid-cols-2 gap-3">
              <FlatCard className="p-3">
                <p className="text-[9px] font-bold uppercase tracking-wider" style={{ color: "#1b365d" }}>Employee</p>
                <p className="text-[15px] font-black" style={{ color: "#059669", fontVariantNumeric: "tabular-nums" }}>{fmtRupee(viewDetailsRow.empCost)}</p>
              </FlatCard>
              <FlatCard className="p-3">
                <p className="text-[9px] font-bold uppercase tracking-wider" style={{ color: "#1b365d" }}>Direct</p>
                <p className="text-[15px] font-black" style={{ color: "#6366F1", fontVariantNumeric: "tabular-nums" }}>{fmtRupee(viewDetailsRow.direct)}</p>
              </FlatCard>
              <FlatCard className="p-3">
                <p className="text-[9px] font-bold uppercase tracking-wider" style={{ color: "#1b365d" }}>Common</p>
                <p className="text-[15px] font-black" style={{ color: "#8B5CF6", fontVariantNumeric: "tabular-nums" }}>{fmtRupee(viewDetailsRow.overhead)}</p>
              </FlatCard>
              <FlatCard className="p-3">
                <p className="text-[9px] font-bold uppercase tracking-wider" style={{ color: "#1b365d" }}>Total</p>
                <p className="text-[15px] font-black" style={{ color: "#DE1A1A", fontVariantNumeric: "tabular-nums" }}>{fmtRupee(viewDetailsRow.total)}</p>
              </FlatCard>
            </div>
            <div>
              <p className="text-[11px] font-black uppercase tracking-wider mb-2" style={{ color: "#1b365d" }}>Direct expense entries this month</p>
              {viewDetailsEntries.length === 0 ? (
                <p className="text-[12px]" style={{ color: "#1b365d" }}>No direct expenses logged for this client.</p>
              ) : (
                  ...
                  {viewDetailsEntries.map(e => (
                    <FlatCard key={e.id} className="p-3 flex items-center justify-between">
                      <div>
                        ...
                        <p className="text-[10px] font-semibold" style={{ color: "#1b365d" }}>{fmtDate(e.date)}{e.notes ? ` · ${e.notes}` : ""}</p>
                      </div>
